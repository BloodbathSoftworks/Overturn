...
void CFuncRotating :: RampPitchVol (int fUp)
{
	...
	EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char *)STRING(pev->noiseRunning), 
		fvol, m_flAttenuation, SND_CHANGE_PITCH/* | SND_CHANGE_VOL*/ | SND_AMBIENT, pitch);
...
void CFuncRotating :: SpinUp( void )
{
	...
	if ( m_fCurSpeed >= pev->speed )
	{
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char *)STRING(pev->noiseRunning), 
			m_flVolume, m_flAttenuation, SND_CHANGE_PITCH/* | SND_CHANGE_VOL*/ | SND_AMBIENT, FANPITCHMAX);
...
//		if ( pev->avelocity != g_vecZero )
		...
		else// fan is not moving, so start it
		{
			...
			SetThink(&CFuncRotating :: SpinUp );
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char *)STRING(pev->noiseRunning), 
				/*0.01*/m_flVolume, m_flAttenuation, SND_AMBIENT, FANPITCHMIN);
...
//		if ( pev->avelocity != g_vecZero )
		...
		else
		{
			m_iState = STATE_ON;
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char *)STRING(pev->noiseRunning), 
				m_flVolume, m_flAttenuation, SND_AMBIENT, FANPITCHMAX);

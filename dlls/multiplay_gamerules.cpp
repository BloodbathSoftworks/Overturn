...
void CHalfLifeMultiplay :: PlayerSpawn( CBasePlayer *pPlayer )
{
	...
	if ( addDefault )
	{
		pPlayer->GiveNamedItem( "weapon_powerglove" );
		pPlayer->GiveNamedItem( "weapon_stakeout" );

		pPlayer->GiveAmmo( 12, "20buckshot", _20BUCKSHOT_MAX_CARRY );// 4 full reloads
	}
}
...
void CHalfLifeMultiplay::DeathNotice( CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pevInflictor )
{
	...
	else if ( strncmp( killer_weapon_name, "trigger_", 8 ) == 0 )
		killer_weapon_name += 8;
...
/*	if ( pKiller->flags & FL_MONSTER )
	...
	if ( pKiller->flags & FL_MONSTER )
	{
		sprintf(szText,"%s was killed by a monster.");
	}
	else if ( pKiller == pVictim->pev )
	{
		sprintf(szText,"%s commited suicide.");
	}
	else if ( pKiller->flags & FL_CLIENT )
	{
		sprintf(szText,"%s killed %s with %s.");
	}
	else if ( FClassnameIs ( pKiller, "worldspawn" ) )
	{
		sprintf(szText,"%s fell or drowned or something.");
	}
	else if ( pKiller->solid == SOLID_BSP )
	{
		sprintf(szText,"%s was mooshed.");
	}
	else
	{
		sprintf(szText,"%s died mysteriously.");
	}

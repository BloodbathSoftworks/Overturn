...
void CGauss::Precache( void )
{
	...
	PRECACHE_SOUND("weapons/gauss1.wav");
	...
	PRECACHE_SOUND("weapons/gaussspin.wav");
...
{
	PLAYBACK_EVENT_FULL( 0, m_pPlayer->edict(), m_usGaussFire, 0.01, (float *)&m_pPlayer->pev->origin, (float *)&m_pPlayer->pev->angles, 0.0, 0.0, 0, 0, 0, 1 );
...
void CGauss::SecondaryAttack()
{
	...
	if ( g_pGameRules->IsMultiplayer() )
		...
		if ( m_pPlayer->m_flStartCharge < gpGlobals->time - 10 )
		{
			...
			EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/electro5.wav", 1.0, ATTN_NORM, 0, 80 + RANDOM_LONG(0,0x3f));
...
void CGauss::WeaponIdle( void )
{
	...
	if ( m_pPlayer->m_flPlayAftershock && m_pPlayer->m_flPlayAftershock < gpGlobals->time )
	{
		...
		case 3:	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/beamstart3.wav", RANDOM_FLOAT(0.7, 0.8), ATTN_NORM); break;
		case 4:	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/beamstart14.wav", RANDOM_FLOAT(0.7, 0.8), ATTN_NORM); break;
		case 5:	EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/beamstart15.wav", RANDOM_FLOAT(0.7, 0.8), ATTN_NORM); break;
		case 6:	break; // no sound
		}
		m_pPlayer->m_flPlayAftershock = 0.0;
	}

...
void DLLEXPORT HUD_TxferLocalOverrides( struct entity_state_s *state, const struct clientdata_s *client )
{
	...
	//overturn
	VectorCopy( client->vuser1, state->vuser1 );
	VectorCopy( client->vuser2, state->vuser2 );
	VectorCopy( client->vuser3, state->vuser3 );
	VectorCopy( client->vuser4, state->vuser4 );
}
...
void DLLEXPORT HUD_ProcessPlayerState( struct entity_state_s *dst, const struct entity_state_s *src )
{
	...
	if ( dst->number == player->index )
	{
		...
		//JIW - FOV
		g_iInZoom = src->vuser1[0];
		g_flFOV = src->vuser1[1];
		g_iRoomType = src->vuser1[2];//JIW
	}
}
...
void DLLEXPORT HUD_StudioEvent( const struct mstudioevent_s *event, const struct cl_entity_s *entity )
{
	...
	// Client side sound
	case 5004:		
		gSoundSystem.PlaySound((char *)event->options, (float *)&entity->attachment[0], 0, CHAN_WEAPON);
		break;
	default:
		break;
	}
...
// Experiment
			...
			if ( pTemp->flags & (FTENT_COLLIDEALL | FTENT_COLLIDEWORLD) )
			{
				...
				if ( traceFraction != 1 )	// Decent collision now, and damping works
				{
					...
					{
						//FMOD SOUND ENGINE
						gSoundSystem.TempEntPlaySound(pTemp, damp);
						//FMOD SOUND ENGINE
					}

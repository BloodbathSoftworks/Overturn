...
ammo_check_t ammo_check[] = {
	{"ammo_canister", "Snarks", FLAME_MAX_CARRY},
	{"ammo_flarebox", "Hornets", FLARE_MAX_CARRY},
	{"ammo_grenades", "rockets", ROCKET_MAX_CARRY},
	{"ammo_9mmclip", "9mm", _9MM_MAX_CARRY},
	{"ammo_m16clip", "10mm", _10MM_MAX_CARRY},
	{"ammo_357", "357", _357_MAX_CARRY},
	{"ammo_12buckshot", "12buckshot", _12BUCKSHOT_MAX_CARRY},
	{"ammo_20buckshot", "20buckshot", _20BUCKSHOT_MAX_CARRY},
...
//         primary_ammo[WEAPON_MP5] = m_rgAmmo[i];
   ...
   //      secondary_ammo[WEAPON_MP5] = m_rgAmmo[i];
    /*  else if (strcmp("bolts", CBasePlayerItem::AmmoInfoArray[i].pszName) == 0)
      ...
      else if (stricmp("12buckshot", CBasePlayerItem::AmmoInfoArray[i].pszName) == 0)
      {
         primary_ammo[WEAPON_SHOTGUN] = m_rgAmmo[i];
      }
      else if (stricmp("20buckshot", CBasePlayerItem::AmmoInfoArray[i].pszName) == 0)
      {
         primary_ammo[WEAPON_SHOTGUN] = m_rgAmmo[i];
      }
      ...
      /*else if (strcmp("uranium", CBasePlayerItem::AmmoInfoArray[i].pszName) == 0)
      {
         ...
         primary_ammo[WEAPON_FLAREGUN] = m_rgAmmo[i];
 ...
 //     else if (stricmp("Satchel Charge", CBasePlayerItem::AmmoInfoArray[i].pszName) == 0)
  //       primary_ammo[WEAPON_SATCHEL] = m_rgAmmo[i];
         ...
         primary_ammo[WEAPON_FLAMETHROWER] = m_rgAmmo[i];
   }
...
BOOL CBot::BotFireWeapon( Vector v_enemy_origin, int weapon_choice, BOOL primary )
{
   ...
   if (pev->weapons & (1<<WEAPON_POWERGLOVE))
   {
      ...
      if (((distance <= 64) && (bot_skill <= 2) && (weapon_choice == 0)) ||
          (weapon_choice == WEAPON_POWERGLOVE))
      {
         new_weapon = weapon_ptr[WEAPON_POWERGLOVE];

         // check if the bot isn't already using this item...
         if (m_pActiveItem != new_weapon)
            SelectItem("weapon_powerglove");  // select the crowbar
...
// BigGuy - END
   ...
   if ((pev->weapons & (1<<WEAPON_STAKEOUT)) && (pev->waterlevel != 3))
   {
      // if close enough for good shotgun blasts...
      if (((distance > 30) && (distance < 150) && (weapon_choice == 0)) ||
          (weapon_choice == WEAPON_STAKEOUT))
      {
         new_weapon = weapon_ptr[WEAPON_STAKEOUT];

         // check if the bot has any ammo left for this weapon...
         if (primary_ammo[WEAPON_STAKEOUT] > 0)
         {
            // check if the bot isn't already using this item...
            if (m_pActiveItem != new_weapon)
               SelectItem("weapon_stakeout");  // select the shotgun
            ...
            {
// BigGuy - START
               pev->button |= IN_ATTACK;  // use secondary attack (bang! bang!)
...
*/
   ...
   if ((pev->weapons & (1<<WEAPON_M16A2)) && (pev->waterlevel != 3))
   {
      long use_secondary = RANDOM_LONG(1,100);

      // if close enough for good MP5 shot...
      if (((distance < 400) && (weapon_choice == 0)) ||
          (weapon_choice == WEAPON_M16A2))
      {
         new_weapon = weapon_ptr[WEAPON_M16A2];

         // check if the bot has any ammo left for this weapon...
         if (primary_ammo[WEAPON_M16A2] > 0)
         {
            // check if the bot isn't already using this item...
            if (m_pActiveItem != new_weapon)
               SelectItem("weapon_m16a2");  // select the 9mmAR (MP5)
   ...
   if (pev->weapons & (1<<WEAPON_RPG))
   {
          ...
          (weapon_choice == WEAPON_RPG))
      {
         ...
         if (primary_ammo[WEAPON_RPG] > 0)
         {
            // check if the bot isn't already using this item...
            if (m_pActiveItem != new_weapon)
               SelectItem("weapon_glauncher");  // select the RPG rocket launcher
	...
	if (pev->weapons & (1<<WEAPON_FLAMETHROWER))
   {
      // don't use the RPG unless the enemy is pretty far away...
      if (((distance > 300) && (weapon_choice == 0)) ||
          (weapon_choice == WEAPON_FLAMETHROWER))
      {
         new_weapon = weapon_ptr[WEAPON_FLAMETHROWER];

         // check if the bot has any ammo left for this weapon...
         if (primary_ammo[WEAPON_FLAMETHROWER] > 0)
         {
            // check if the bot isn't already using this item...
            if (m_pActiveItem != new_weapon)
               SelectItem("weapon_flamethrower");  // select the RPG rocket launcher
	...
	if (pev->weapons & (1<<WEAPON_FLAREGUN))
   {
      // don't use the RPG unless the enemy is pretty far away...
      if (((distance > 100) && (weapon_choice == 0)) ||
          (weapon_choice == WEAPON_FLAREGUN))
      {
         new_weapon = weapon_ptr[WEAPON_FLAREGUN];

         // check if the bot has any ammo left for this weapon...
         if (primary_ammo[WEAPON_FLAREGUN] > 0)
         {
            // check if the bot isn't already using this item...
            if (m_pActiveItem != new_weapon)
               SelectItem("weapon_flaregun");  // select the RPG rocket launcher

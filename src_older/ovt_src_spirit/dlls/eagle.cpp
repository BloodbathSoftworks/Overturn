...
int CDeagle::GetItemInfo(ItemInfo *p)
{
	...
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = WEAPON_NOCLIP;//EAGLE_MAX_CLIP;
	p->iSlot = 1;
	p->iPosition = 2;
	p->iFlags = 0;
...
/*	if (m_pEagleLaser)
	{
		...
		m_flNextPrimaryAttack = gpGlobals->time + 0.50;
#ifdef PARTICLEFUSION
	...
	{
		flSpread = 0.1;
		vecDir = m_pPlayer->FireBulletsPlayer( 1, vecSrc, vecAiming, VECTOR_CONE_10DEGREES, 8192, BULLET_PLAYER_EAGLE, 0, 0, m_pPlayer->pev, m_pPlayer->random_seed );
#ifdef PARTICLEFUSION
		m_flNextPrimaryAttack = gpGlobals->time + 0.20;
		m_fLaserOn = FALSE;
#endif
	}

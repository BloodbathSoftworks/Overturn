...
#define IMPLEMENT_SAVERESTORE(derivedClass,baseClass) \
	...
	{\
		if ( !baseClass::Save(save) )\
			return 0;\
		if (pev->targetname)\
			return save.WriteFields( STRING(pev->targetname), #derivedClass, this, m_SaveData, ARRAYSIZE_HL(m_SaveData) );\
		else\
			return save.WriteFields( STRING(pev->classname), #derivedClass, this, m_SaveData, ARRAYSIZE_HL(m_SaveData) );\
	}\
	int derivedClass::Restore( CRestore &restore )\
	{\
		if ( !baseClass::Restore(restore) )\
			return 0;\
		return restore.ReadFields( #derivedClass, this, m_SaveData, ARRAYSIZE_HL(m_SaveData) );\
	}

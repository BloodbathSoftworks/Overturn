...
float V_CalcBob2 ( struct ref_params_s *pparams )
{
	...
	bobtime += pparams->frametime /1.5;
	cycle = bobtime - (int)( bobtime / cl_bobcycle2->value ) * cl_bobcycle2->value;
	cycle /= cl_bobcycle2->value;
	
	if ( cycle < cl_bobup2->value )
	{
		cycle = M_PI * cycle / cl_bobup2->value;
	}
	else
	{
		cycle = M_PI + M_PI * ( cycle - cl_bobup2->value )/( 1.0 - cl_bobup2->value );
	}
	...
	bob = sqrt( vel[0] * vel[0] + vel[1] * vel[1] ) * cl_bob2->value;
	bob = bob * 0.3 + bob * 0.7 * sin(cycle);
	bob = min( bob, 4 );
	bob = max( bob, -7 );
	return bob;
...
float V_CalcBob_Camera ( struct ref_params_s *pparams )
{
	static	double	bobtime;
	static float	bob;
	float	cycle;
...
float V_CalcBob_Camera2 ( struct ref_params_s *pparams )
{
	static	double	bobtime;
	static float	bob;
	float	cycle;
...
float V_CalcBob_lowered ( struct ref_params_s *pparams )
{
	static	double	bobtime;
	static float	bob;
	float	cycle;
	...
	bobtime += pparams->frametime /4;
	cycle = bobtime - (int)( bobtime / cl_bobcycle2->value ) * cl_bobcycle2->value;
	cycle /= cl_bobcycle2->value;
	

	cycle = M_PI * cycle / cl_bobup3->value *2;
	...
	bob = sqrt( vel[0] * vel[0] + vel[1] * vel[1] ) * cl_bob3->value;
	bob = bob * 0.3 + bob * 0.7;
	bob = min( bob, 4 );
	bob = max( bob, -7 );
	return bob;
...
void V_AddIdle ( struct ref_params_s *pparams )
{
	...
	view->origin[ROLL] += v_idlescale * sin(pparams->time*v_iroll_cycle.value) * v_iroll_level.value;
	view->origin[PITCH] += v_idlescale * sin(pparams->time*v_ipitch_cycle.value) * v_ipitch_level.value;
	view->origin[YAW] += v_idlescale * sin(pparams->time*v_iyaw_cycle.value) * v_iyaw_level.value;

	pparams->viewangles[ROLL] += v_idlescale * sin(pparams->time*v_iroll_cycle.value) * v_iroll_level.value * 0.7;
	pparams->viewangles[PITCH] += v_idlescale * sin(pparams->time*v_ipitch_cycle.value) * v_ipitch_level.value * 0.7;
	pparams->viewangles[YAW] += v_idlescale * sin(pparams->time*v_iyaw_cycle.value) * v_iyaw_level.value * 0.7;
}
...
void V_CalcViewRoll ( struct ref_params_s *pparams )
{
	...
	if ( pparams->health <= 0 && ( pparams->viewheight[2] != 0 ) )
	{
		...
		pparams->viewangles[ROLL] = 40;
		pparams->viewangles[PITCH] = -15;
		return;
	}
}
...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
	...
	int				i;
	vec3_t			angles;
	float			bob, bob2, bob_cam, bob_cam2, waterOffset;
	...
	bob_cam = V_CalcBob_Camera ( pparams );
	bob_cam2 = V_CalcBob_Camera2 ( pparams );
	V_CalcViewRoll ( pparams );

	if(gHUD.m_pCvarViewSway->value >= 1)
	{
		v_idlescale = 2.5;
	}
	else
	...
	for ( i = 0; i < 3; i++ )
	{
		//view->origin[i] += pparams->right[i]*bob2*0.2;
		//view->origin[i] += pparams->forward[i]*bob*0.2;



		view->origin[ROLL] += bob *0.05;
		view->origin[YAW] += bob *0.05;

		pparams->viewangles[i] += pparams->right[i]*bob_cam2*0.1;
		pparams->viewangles[i] += pparams->up[i]*bob_cam*0.1;
	}
	view->origin[2] += bob;
...
int V_FindViewModelByWeaponModel(int weaponindex)
{
	...
	static char * modelmap[][2] =	{
		...
		{ "models/p_stakeout.mdl",		"models/v_stakeout.mdl"		},
...
void DLLEXPORT V_CalcRefdef( struct ref_params_s *pparams )
{
	...
	gSoundSystem.CalcRefDef(pparams);
	memcpy(&gHUD.m_pParams, pparams, sizeof(ref_params_t));
/*
...
void V_Init (void)
{
	...
	cl_bobcycle			= gEngfuncs.pfnRegisterVariable( "cl_bobcycle","0.43", 0 );
	cl_bob				= gEngfuncs.pfnRegisterVariable( "cl_bob","-0.01", 0 );
	cl_bobup			= gEngfuncs.pfnRegisterVariable( "cl_bobup","0.45", 0 );

	cl_bobcycle2			= gEngfuncs.pfnRegisterVariable( "cl_bobsidecycle","0.86", 0 );
	cl_bob2			= gEngfuncs.pfnRegisterVariable( "cl_bobside","-0.013", 0 );
	cl_bobup2			= gEngfuncs.pfnRegisterVariable( "cl_bobsideup","0.5", 0 );

	cl_bob3			= gEngfuncs.pfnRegisterVariable( "cl_bob3","-0.025", 0 );
	cl_bobup3			= gEngfuncs.pfnRegisterVariable( "cl_bob3up","0.5", 0 );

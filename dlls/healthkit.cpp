...
BOOL CHealthKit::MyTouch( CBasePlayer *pPlayer )
{
	...
	if ( !(pPlayer->m_afButtonPressed & IN_USE) && ( !g_pGameRules->IsMultiplayer() ))
	{
		...
		{
		UTIL_CustomMessage( pPlayer, MSG_GAME, "PRESS [Use] TO PICKUP THE MEDKIT");
	...
	{
		EMIT_SOUND_DYN(ENT(pPlayer->pev), CHAN_ITEM, "items/smallmedkit1.wav", 1, ATTN_NORM, 0, 100);
	...
	else if ( pPlayer->m_afButtonPressed & IN_USE && (pPlayer->medkits < MAX_PLAYER_MEDKITS) )
	{
		pPlayer->medkits++;

...
BOOL CHealthKit::MyTouch( CBasePlayer *pPlayer )
{
	...
	if ( pPlayer->m_afButtonPressed & IN_USE && (pPlayer->medkits < MAX_PLAYER_MEDKITS) )//5) )
	{
		pPlayer->medkits++;

		MESSAGE_BEGIN( MSG_ONE, gmsgMedkit, NULL, pPlayer->pev );
			WRITE_BYTE( pPlayer->medkits );
		MESSAGE_END();

		EMIT_SOUND_DYN(ENT(pPlayer->pev), CHAN_ITEM, "items/smallmedkit1.wav", 1, ATTN_NORM, SND_AMBIENT, 100);

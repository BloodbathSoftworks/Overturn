...
// mp3 player, killar
#define SF_FMOD_REMOVE_FIRE	1
#define SF_FMOD_LOOPED		2

extern int gmsgPlayAudio; //AJH - Killars MP3player
...
{
	DEFINE_FIELD( CTargetFMODAudio, m_bActive, FIELD_BOOLEAN ),
	DEFINE_FIELD( CTargetFMODAudio, m_flStartTime, FIELD_TIME ),
};
...
void CTargetFMODAudio :: Precache( void )
{
...
{
	MESSAGE_BEGIN( MSG_ALL, gmsgPlayAudio, NULL );
		WRITE_STRING( STRING(pev->message) );
		WRITE_BYTE((pev->spawnflags & SF_FMOD_LOOPED) ? SF_FMOD_LOOPED : NULL);
	MESSAGE_END();
	m_bActive = TRUE;

	// remove if set
	if(FBitSet(pev->spawnflags, SF_FMOD_REMOVE_FIRE))
		UTIL_Remove(this);
}

void CTargetFMODAudio::SendInitMessage(CBasePlayer *player)
{
	if(!FBitSet(pev->spawnflags, SF_FMOD_LOOPED) || !m_bActive)
		return;

	MESSAGE_BEGIN( MSG_ALL, gmsgPlayAudio, NULL );
		WRITE_STRING(STRING(pev->message));
		WRITE_BYTE(SF_FMOD_LOOPED);
	MESSAGE_END();
}
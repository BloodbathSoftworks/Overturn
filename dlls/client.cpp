...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq(pcmd, "tabmenu" ) )
    {
            GetClassPtr((CBasePlayer *)pev)->ShowTABMenu();
    }
	else if ( FStrEq(pcmd, "use_medkit" ) )
    {
            GetClassPtr((CBasePlayer *)pev)->UseMedkit();
    }
	else if ( FStrEq(pcmd, "use_battery" ) )
    {
            GetClassPtr((CBasePlayer *)pev)->UseBattery();
    }

	//Audio stuff
   else if ( FStrEq(pcmd, "setroomtype" ) )
   {
	   if (g_flWeaponCheat != 0.0)
	   {
		CBasePlayer * pPlayer = GetClassPtr((CBasePlayer *)pev);

		if (pPlayer && CMD_ARGC() > 1)
		{
			pPlayer->m_iCurrentRoomType = atoi( CMD_ARGV(1) );
			pPlayer->m_bUpdateRoomType = TRUE;
			CLIENT_PRINTF( pEntity, print_console, UTIL_VarArgs("\"setroomtype\" set to %d\n", pPlayer->m_iCurrentRoomType) );
		}
		else
			CLIENT_PRINTF( pEntity, print_console, UTIL_VarArgs("\"setroomtype\" is %d\n", pPlayer->m_iCurrentRoomType) );
	   }
	}
...
// player pain sounds
	PRECACHE_SOUND("player/jumpshort02.wav");
	PRECACHE_SOUND("player/jumpshort07.wav");
	PRECACHE_SOUND("player/jumpshort16.wav");
	PRECACHE_SOUND("player/jumpshort19.wav");
	PRECACHE_SOUND("player/jumpshort29.wav");
	...
	PRECACHE_SOUND("player/land1.wav");
	...
	PRECACHE_SOUND("player/pl_step-jump1.wav");//jumping sounds
	PRECACHE_SOUND("player/pl_dirt-jump1.wav");
	PRECACHE_SOUND("player/pl_metal-jump1.wav");

	PRECACHE_SOUND("player/pl_step-land1.wav");//landing sounds
	PRECACHE_SOUND("player/pl_dirt-land1.wav");
	PRECACHE_SOUND("player/pl_metal-land1.wav");
...
int AddToFullPack( struct entity_state_s *state, int e, edict_t *ent, edict_t *host, int hostflags, int player, unsigned char *pSet )
{
	...
	if ( ent != host && !(ent->v.effects & EF_NOVIS))
	{
...
void UpdateClientData ( const struct edict_s *ent, int sendweapons, struct clientdata_s *cd )
{
	...
	if (pl)
	{
		cd->vuser1[0] = pl->g_iInZoom;
		cd->vuser1[1] = pl->g_flFOV;	
		cd->vuser1[2] = pl->pev->vuser1[2];
	}
}

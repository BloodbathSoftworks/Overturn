...
void CISlave :: PainSound( void )
{
	if (RANDOM_LONG( 0, 2 ) == 0)
	{
		EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pPainSounds[ RANDOM_LONG(0,ARRAYSIZE_HL(pPainSounds)-1) ], 1.0, ATTN_NORM, 0, m_voicePitch );
	}
}
...
{
	EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pDeathSounds[ RANDOM_LONG(0,ARRAYSIZE_HL(pDeathSounds)-1) ], 1.0, ATTN_NORM, 0, m_voicePitch );
}
	...
	switch( pEvent->event )
	{
		case ISLAVE_AE_CLAW:
		{
			...
			if ( pHurt )
			{
				...
				EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pAttackHitSounds[ RANDOM_LONG(0,ARRAYSIZE_HL(pAttackHitSounds)-1) ], 1.0, ATTN_NORM, 0, m_voicePitch );
			}
			else
			{
				// Play a random attack miss sound
				EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pAttackMissSounds[ RANDOM_LONG(0,ARRAYSIZE_HL(pAttackMissSounds)-1) ], 1.0, ATTN_NORM, 0, m_voicePitch );
			}
		}
		break;
		...
		case ISLAVE_AE_ZAP_SHOOT:
		{
			...
			if (m_hDead != NULL)
			{
				...
				if ( !trace.fStartSolid )
				{
					...
					WackBeam( 1, pNew );
					UTIL_Remove( m_hDead );
					EMIT_SOUND_DYN( ENT(pev), CHAN_WEAPON, "hassault/hw_gun4.wav", 1, ATTN_NORM, 0, RANDOM_LONG( 130, 160 ) );
...
void CISlave :: Precache()
{
	...
	for ( i = 0; i < ARRAYSIZE_HL( pDeathSounds ); i++ )

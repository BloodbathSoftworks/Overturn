...
void CAGrunt :: PrescheduleThink ( void )
{
	if ( ShouldSpeak() )
	{
		if ( m_flNextWordTime < gpGlobals->time )
		{
			...
			do
			{
				num = RANDOM_LONG(0,ARRAYSIZE_HL(pIdleSounds)-1);
...
void CAGrunt :: DeathSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pDieSounds[RANDOM_LONG(0,ARRAYSIZE_HL(pDieSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AlertSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAlertSounds[RANDOM_LONG(0,ARRAYSIZE_HL(pAlertSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AttackSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAttackSounds[RANDOM_LONG(0,ARRAYSIZE_HL(pAttackSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: PainSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pPainSounds[RANDOM_LONG(0,ARRAYSIZE_HL(pPainSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case AGRUNT_AE_LEFT_FOOT:
		...
		{
		// left foot
		case 0:	EMIT_SOUND_DYN ( ENT(pev), CHAN_BODY, "agrunt/ag_step2.wav", 1, ATTN_NORM, 0, 70 );	break;
		case 1:	EMIT_SOUND_DYN ( ENT(pev), CHAN_BODY, "agrunt/ag_step4.wav", 1, ATTN_NORM, 0, 70 );	break;
		}
		break;
	case AGRUNT_AE_RIGHT_FOOT:
		...
		switch (RANDOM_LONG(0,1))
		{
		case 0:	EMIT_SOUND_DYN ( ENT(pev), CHAN_BODY, "agrunt/ag_step1.wav", 1, ATTN_NORM, 0, 70 );	break;
		case 1:	EMIT_SOUND_DYN ( ENT(pev), CHAN_BODY, "agrunt/ag_step3.wav", 1, ATTN_NORM, 0 ,70);	break;
		}
		break;
	...
	case AGRUNT_AE_LEFT_PUNCH:
		{
			...
			if ( pHurt )
			{
				...
				EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pAttackHitSounds[ RANDOM_LONG(0,ARRAYSIZE_HL(pAttackHitSounds)-1) ], 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5,5) );
			...
			else
			{
				...
				EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pAttackMissSounds[ RANDOM_LONG(0,ARRAYSIZE_HL(pAttackMissSounds)-1) ], 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5,5) );
			}
		}
		break;
...
void CAGrunt :: Precache()
{
	...
	for ( i = 0; i < ARRAYSIZE_HL( pAttackHitSounds ); i++ )
		PRECACHE_SOUND((char *)pAttackHitSounds[i]);

	for ( i = 0; i < ARRAYSIZE_HL( pAttackMissSounds ); i++ )
		PRECACHE_SOUND((char *)pAttackMissSounds[i]);

	for ( i = 0; i < ARRAYSIZE_HL( pIdleSounds ); i++ )
		PRECACHE_SOUND((char *)pIdleSounds[i]);

	for ( i = 0; i < ARRAYSIZE_HL( pDieSounds ); i++ )
		PRECACHE_SOUND((char *)pDieSounds[i]);

	for ( i = 0; i < ARRAYSIZE_HL( pPainSounds ); i++ )
		PRECACHE_SOUND((char *)pPainSounds[i]);

	for ( i = 0; i < ARRAYSIZE_HL( pAttackSounds ); i++ )
		PRECACHE_SOUND((char *)pAttackSounds[i]);

	for ( i = 0; i < ARRAYSIZE_HL( pAlertSounds ); i++ )
...
Schedule_t	slAGruntCombatFail[] =
{
	{
		tlAGruntCombatFail,
		ARRAYSIZE_HL ( tlAGruntCombatFail ),
		bits_COND_CAN_RANGE_ATTACK1	|
		bits_COND_CAN_MELEE_ATTACK1,
		0,
		"AGrunt Combat Fail"
...
{
	{
		...
		ARRAYSIZE_HL ( tlAGruntSuppressHornet ),
		0,
		0,
...
Schedule_t	slAGruntRangeAttack1[] =
{
	{ 
		...
		ARRAYSIZE_HL ( tlAGruntRangeAttack1 ), 
		bits_COND_NEW_ENEMY			|
...
Schedule_t	slAGruntHiddenRangeAttack[] =
{
	{ 
		...
		ARRAYSIZE_HL ( tlAGruntHiddenRangeAttack1 ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_HEAVY_DAMAGE		|
		bits_COND_HEAR_SOUND,
...
Schedule_t	slAGruntTakeCoverFromEnemy[] =
{
	{ 
		...
		ARRAYSIZE_HL ( tlAGruntTakeCoverFromEnemy ), 
		bits_COND_NEW_ENEMY,
		0,
...
Schedule_t	slAGruntVictoryDance[] =
{
	{ 
		...
		ARRAYSIZE_HL ( tlAGruntVictoryDance ), 
		bits_COND_NEW_ENEMY		|
...
Schedule_t	slAGruntThreatDisplay[] =
{
	{ 
		...
		ARRAYSIZE_HL ( tlAGruntThreatDisplay ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE,

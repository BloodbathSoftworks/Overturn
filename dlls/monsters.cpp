...
int CBaseMonster::Save( CSave &save )
{
	if ( !CBaseToggle::Save(save) )
		return 0;
	if ( pev->targetname )
		return save.WriteFields( STRING(pev->targetname), "CBaseMonster", this, m_SaveData, ARRAYSIZE_HL(m_SaveData) );
	else
		return save.WriteFields( STRING(pev->classname), "CBaseMonster", this, m_SaveData, ARRAYSIZE_HL(m_SaveData) );
}
...
int CBaseMonster::Restore( CRestore &restore )
{
	if ( !CBaseToggle::Restore(restore) )
		return 0;
	int status = restore.ReadFields( "CBaseMonster", this, m_SaveData, ARRAYSIZE_HL(m_SaveData) );
...
void CBaseMonster::ReportAIState( void )
{
	...
	if ( (int)m_MonsterState < ARRAYSIZE_HL(pStateNames) )

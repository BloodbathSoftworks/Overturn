...
private:
	...
	void		 CreateTABMenu( void );
	CMenuPanel*  ShowTABMenu( void );
...
public:
	...
	int MsgFunc_TABMenu( const char *pszName, int iSize, void *pbuf );
...
class CTABMenuPanel : public CMenuPanel
{
private:
    CommandButton       *m_pCancelButton;
	CommandButton		*m_pSaveButton;
	CommandButton		*m_pLoadButton;
	CommandButton		*m_pInfoButton;
public:
	CTABMenuPanel(int iTrans, int iRemoveMe, int x, int y, int wide, int tall);
};
...
public:
	...
	void paint()
	{
		...
		if (gHUD.m_Health.m_fFade)
		{
			...
			if (gOverturnHUD.m_Health.m_fFade <= 0)
			{
				a = MIN_ALPHA;

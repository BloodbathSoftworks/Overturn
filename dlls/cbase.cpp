...
//	DEFINE_FIELD( CBaseEntity, m_pAssistLink, FIELD_CLASSPTR ), //LRC - don't save this, we'll just rebuild the list on restore
	...
	DEFINE_FIELD(CBaseEntity, m_flBurnTime, FIELD_TIME),//XDM
	DEFINE_FIELD(CBaseEntity, m_flNextBurnTime, FIELD_TIME),//XDM
	DEFINE_FIELD(CBaseEntity, m_pFireAttacker, FIELD_EHANDLE),//XDM
};
...
int CBaseEntity::Save( CSave &save )
{
	...
	{
		if (pev->targetname)
			return save.WriteFields( STRING(pev->targetname), "BASE", this, m_SaveData, ARRAYSIZE_HL(m_SaveData) );
		else
			return save.WriteFields( STRING(pev->classname), "BASE", this, m_SaveData, ARRAYSIZE_HL(m_SaveData) );
	}
...
int CBaseEntity::Restore( CRestore &restore )
{
	...
	if ( status )
		status = restore.ReadFields( "BASE", this, m_SaveData, ARRAYSIZE_HL(m_SaveData) );

...
void CAmbientGeneric :: SendInitMessage( CBasePlayer *player )
{
	char* szSoundFile = (char*) STRING(pev->message);
	...
	if(player->m_bHadFirstSpawn)//only happens on the player's first spawn
		return;

//ALERT(at_console,"%s loop: %d active: %d\n", szSoundFile,m_fLooping,m_fActive);

	EMIT_AMBIENT_SOUND ( ENT(pev), pev->origin, szSoundFile, (m_dpv.vol * 0.01), m_flAttenuation, SND_SPAWNING|SND_AMBIENT, m_dpv.pitch, player->entindex());
...
void CEnvSound :: Think( void )
{
	...
	if (FEnvSoundInRange(pev, VARS(pentPlayer), &flRange)) 
	{
		if (flRange < pPlayer->m_flSndRange || pPlayer->m_flSndRange == 0) 
		{
			...
			//FMOD SOUND ENGINE
			pPlayer->m_iCurrentRoomType = (int)m_flRoomtype;

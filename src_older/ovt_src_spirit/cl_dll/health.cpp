...
int CHudHealth::Draw(float flTime)
{
	...
	if (m_fFade)
	{
		...
		a = MIN_ALPHA +  (m_fFade/FADE_TIME) * 255;
	...
	if (gHUD.m_iWeaponBits & (1<<(WEAPON_SUIT)))
	{
		...
		y = y - (gHUD.GetSpriteRect(m_HUD_healthbar).bottom - gHUD.GetSpriteRect(m_HUD_healthbar).top) - 5;
		x = x + 7;
		...
		if (iOffset < m_iWidth)
		{
			...
			SPR_Set(gHUD.GetSprite(m_HUD_healthbar), 255, 255, 255 );
			SPR_Draw(0, x, y, &rc);
		...
		//x = gHUD.DrawHudNumber(x, y-30, DHN_3DIGITS | DHN_DRAWZERO, m_iHealth, r, g, b);

		//x += HealthWidth/2;

		//Ivan Drawing
		ix = 0;
		iy = ScreenHeight - (gHUD.GetSpriteRect(m_HUD_ivan).bottom - gHUD.GetSpriteRect(m_HUD_ivan).top);

		SPR_Set(gHUD.GetSprite(m_HUD_ivan), 255, 255, 255);
		SPR_Draw(0, ix, iy, &gHUD.GetSpriteRect(m_HUD_ivan));
...
int CHudHealth::DrawPain(float flTime)
{
	...
	a = 180 +100 -m_iHealth;	// max brightness until then
	...
	if (m_fAttackFront > 0.4)
	{
		...
		FillRGBA(0, 0, ScreenWidth, ScreenHeight, 190 +100 -m_iHealth, 0, 0, a);

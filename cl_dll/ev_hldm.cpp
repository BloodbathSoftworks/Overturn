...
extern void EV_PlaySound ( int ent, float *origin, int channel, const char *sample, float volume, float attenuation, int fFlags, int pitch );
...
void EV_FireStakeoutSingle( struct event_args_s *args  );
void EV_FireStakeoutSemi( struct event_args_s *args  );
...
void EV_FireField( struct event_args_s *args  );
void EV_FlareBurst( struct event_args_s *args  );
void EV_QRocketTrail( struct event_args_s *args  );
void EV_QExplosion( struct event_args_s *args  );
...
//	if ( iRand < (0x7fff/2) )// not every bullet makes a sound.
	{
		switch( gEngfuncs.pfnRandomLong(0,2))//iRand % 5)
		{
		case 0:	EV_PlaySound( -1, pTrace->endpos, 0, "weapons/ric1.wav", 1.0, ATTN_IDLE, 0, PITCH_NORM ); break;
		case 1:	EV_PlaySound( -1, pTrace->endpos, 0, "weapons/ric2.wav", 1.0, ATTN_IDLE, 0, PITCH_NORM ); break;
		case 2:	EV_PlaySound( -1, pTrace->endpos, 0, "weapons/ric3.wav", 1.0, ATTN_IDLE, 0, PITCH_NORM ); break;
//		case 3:	break;//EV_PlaySound( -1, pTrace->endpos, 0, "weapons/ric4.wav", 1.0, ATTN_NORM, 0, PITCH_NORM ); break;
	//	case 4:	break;//EV_PlaySound( -1, pTrace->endpos, 0, "weapons/ric5.wav", 1.0, ATTN_NORM, 0, PITCH_NORM ); break;
		}
	}
...
void EV_HLDM_DecalGunshot( pmtrace_t *pTrace, int iBulletType )
{
	...
	if ( pe && pe->solid == SOLID_BSP )
	{
		...
		case BULLET_PLAYER_12BUCKSHOT:
		case BULLET_PLAYER_20BUCKSHOT:
...
void EV_HLDM_FireBullets( int idx, float *forward, float *right, float *up, int cShots, float *vecSrc, float *vecDirShooting, float flDistance, int iBulletType, int iTracerFreq, int *tracerCount, float flSpreadX, float flSpreadY )
{
	...
	for ( iShot = 1; iShot <= cShots; iShot++ )	
	{
		...
		if ( iBulletType == BULLET_PLAYER_12BUCKSHOT || iBulletType == BULLET_PLAYER_20BUCKSHOT )
		{
			do {
...
void EV_FireShotGunDouble( event_args_t *args )
{
	...
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 8, vecSrc, vecAiming, 2048, BULLET_PLAYER_12BUCKSHOT, 0, &tracerCount[idx-1], 0.17365, 0.04362 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 12, vecSrc, vecAiming, 2048, BULLET_PLAYER_12BUCKSHOT, 0, &tracerCount[idx-1], 0.08716, 0.08716 );
	}
}
...
void EV_FireShotGunSingle( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 4, vecSrc, vecAiming, 2048, BULLET_PLAYER_12BUCKSHOT, 0, &tracerCount[idx-1], 0.08716, 0.04362 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 6, vecSrc, vecAiming, 2048, BULLET_PLAYER_12BUCKSHOT, 0, &tracerCount[idx-1], 0.08716, 0.08716 );
	}
}
//======================
//	   SHOTGUN END
...
//	  STAKEOUT START
//======================
void EV_FireStakeoutSemi( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
...
//	int j;
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( STAKEOUT_FIRE_SEMI, 2 );
		V_PunchAxis( 0, -10.0 );
	}
	...
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/stakeout_fire2.wav", gEngfuncs.pfnRandomFloat(0.98, 1.0), ATTN_NORM, 0, 85 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 8, vecSrc, vecAiming, 2048, BULLET_PLAYER_20BUCKSHOT, 0, &tracerCount[idx-1], 0.17365, 0.04362 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 12, vecSrc, vecAiming, 2048, BULLET_PLAYER_20BUCKSHOT, 0, &tracerCount[idx-1], 0.08716, 0.08716 );
	}
}

void EV_FireStakeoutSingle( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( STAKEOUT_FIRE, 2 );
	...
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/stakeout_fire1.wav", gEngfuncs.pfnRandomFloat(0.95, 1.0), ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 4, vecSrc, vecAiming, 2048, BULLET_PLAYER_20BUCKSHOT, 0, &tracerCount[idx-1], 0.08716, 0.04362 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 6, vecSrc, vecAiming, 2048, BULLET_PLAYER_20BUCKSHOT, 0, &tracerCount[idx-1], 0.08716, 0.08716 );
	}
}
//======================
//	   STAKEOUT END
...
void EV_FireMP5( event_args_t *args )
{
	...
	case 1:
		...
		break;
	case 2:
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/hks3.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	}
...
void EV_FireM16( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( M16A2_FIRE1 + gEngfuncs.pfnRandomLong(0,2), 2 );
	...
	{
	case 0:
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/colts1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/colts2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 2:
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/colts3.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	}
...
void EV_SpinGauss( event_args_t *args )
{
	...
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/gaussspin.wav", 1.0, ATTN_NORM, iSoundState, pitch );
}
...
void EV_StopPreviousGauss( int idx )
{
	...
	gEngfuncs.pEventAPI->EV_StopSound( idx, CHAN_WEAPON, "weapons/gaussspin.wav" );
}
...
//	Con_Printf( "Firing gauss with %f\n", flDamage );
	...
	{
	case 0:
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/gauss1.wav", 0.5 + flDamage * (1.0 / 400.0), ATTN_NORM, 0, 85 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
		break;
	case 1:
...
void EV_FireCrossbow2( event_args_t *args )
{
	...
	if ( tr.fraction < 1.0 )
	{
		...
		if ( pe->solid != SOLID_BSP )
		{
			...
			{
			case 0:
				EV_PlaySound( idx, tr.endpos, CHAN_BODY, "weapons/xbow_hitbod1.wav", 1, ATTN_NORM, 0, PITCH_NORM ); break;
			case 1:
				EV_PlaySound( idx, tr.endpos, CHAN_BODY, "weapons/xbow_hitbod2.wav", 1, ATTN_NORM, 0, PITCH_NORM ); break;
			}
		}
...
//	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/rocketfire1.wav", 0.9, ATTN_NORM, 0, PITCH_NORM );
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/glauncher.wav", 0.9, ATTN_NORM, 0, PITCH_NORM );

	//Only play the weapon anims if I shot it. 
	if ( EV_IsLocal( idx ) )
	{
		if(gEngfuncs.pfnRandomLong(0,1) == 0)
			gEngfuncs.pEventAPI->EV_WeaponAnimation( GL_SHOOT1, 1 );
		else
			gEngfuncs.pEventAPI->EV_WeaponAnimation( GL_SHOOT2, 1 );
...
void EV_HornetGunFire( event_args_t *args )
{
	...
	switch ( gEngfuncs.pfnRandomLong ( 0 , 2 ) )
	{
		case 0:	EV_PlaySound( idx, origin, CHAN_WEAPON, "agrunt/ag_fire1.wav", 1, ATTN_NORM, 0, 100 );	break;
		case 1:	EV_PlaySound( idx, origin, CHAN_WEAPON, "agrunt/ag_fire2.wav", 1, ATTN_NORM, 0, 100 );	break;
		case 2:	EV_PlaySound( idx, origin, CHAN_WEAPON, "agrunt/ag_fire3.wav", 1, ATTN_NORM, 0, 100 );	break;
	}
}
//======================
//	   HORNET END
...
void EV_FireEagle( event_args_t *args )
{
	...
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/desert_eagle_fire.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );
...
void EV_Knife( event_args_t *args )
{
	...
	switch ( gEngfuncs.pfnRandomLong ( 0 , 2 ) )
	{
		case 0:	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/knife1.wav", 1, ATTN_NORM, 0, 100 );	break;
		case 1:	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/knife2.wav", 1, ATTN_NORM, 0, 100 );	break;
		case 2:	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/knife3.wav", 1, ATTN_NORM, 0, 100 );	break;
	}
...
void EV_FireM249( event_args_t *args )
{
	...
	{
	case 0:
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/saw_fire1.wav", 1, ATTN_NORM, 0,  94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/saw_fire2.wav", 1, ATTN_NORM, 0,  94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 2:
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/saw_fire3.wav", 1, ATTN_NORM, 0,  94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	}
...
void EV_FireM40a1( event_args_t *args )
{
	...
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/sniper_fire.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );
...
void EV_SpinDisplacer(event_args_t *args)//Original code idea by Xwider
{
	...
	if (args->bparam2 == 1)// sound mode
		EV_PlaySound(args->entindex, args->origin, CHAN_WEAPON, "weapons/displacer_spin.wav", 1.0, ATTN_NORM, 0, PITCH_NORM);
	else
		EV_PlaySound(args->entindex, args->origin, CHAN_WEAPON, "weapons/displacer_spin2.wav", 1.0, ATTN_NORM, 0, PITCH_NORM);
}
void EV_FireDisplacer(event_args_t *args)
{
	...
	EV_PlaySound(args->entindex, args->origin, CHAN_WEAPON, "weapons/displacer_fire.wav", 1.0, ATTN_NORM, 0, PITCH_NORM);
}
//======================
//	   DISPLACER END
//======================
...
void EV_SporeLauncherRocket( event_args_t *args )
{
	...
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/splauncher_fire.wav", 0.9, ATTN_NORM, 0, PITCH_NORM );
...
void EV_SporeLauncherGrenade( event_args_t *args )
{
	...
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/splauncher_altfire.wav", 0.9, ATTN_NORM, 0, PITCH_NORM );
...
void EV_FireShock( event_args_t *args )
{
	...
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/shock_fire.wav", 1, ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong(0,0xF) );
...
void EV_TrainPitchAdjust( event_args_t *args )
{
	...
	{
		EV_PlaySound( idx, origin, CHAN_STATIC, sz, m_flVolume, ATTN_NORM, SND_CHANGE_PITCH, pitch );
	}
}
//=============================//
...
////////////////////////
//OVERTURD POOPY BELOW//
////////////////////////

void EV_ExplosionSound( vec3_t org )
{
	switch(gEngfuncs.pfnRandomLong(0, 2))
	{
	case 0: EV_PlaySound(0, org, CHAN_STATIC, "weapons/explode3.wav", 1, 0.3, 0, 100); break;
	case 1: EV_PlaySound(0, org, CHAN_STATIC, "weapons/explode4.wav", 1, 0.3, 0, 100); break;
	case 2: EV_PlaySound(0, org, CHAN_STATIC, "weapons/explode5.wav", 1, 0.3, 0, 100); break;
	}
}
...
void EV_FireFlareGun( event_args_t *args )
{
	...
	EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/flaregun_fire1.wav", 1, ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong(0,0xF) );

	//Only play the weapon anims if I shot it. 
	if ( EV_IsLocal( idx ) )
	{
		//if ( args->iparam1 )
			gEngfuncs.pEventAPI->EV_WeaponAnimation( FGUN_SHOOT, 1 );
		V_PunchAxis( 0, -10.0 );
	}
}
...
void EV_FireFlameThrower( event_args_t *args )
{
	...
	if(gEngfuncs.pfnRandomLong(0,1) == 0)
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/fl_fire1.wav", 1, ATTN_NORM, 0, 97 + gEngfuncs.pfnRandomLong(0,0xF) );
	else
		EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/fl_fire2.wav", 1, ATTN_NORM, 0, 97 + gEngfuncs.pfnRandomLong(0,0xF) );
	...
	if ( EV_IsLocal( idx ) )
	{
		switch(gEngfuncs.pfnRandomLong(0,2))
		{
		case 0: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAME_FIRE1, 1 ); break;
		case 1: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAME_FIRE2, 1 ); break;
		case 2: gEngfuncs.pEventAPI->EV_WeaponAnimation( FLAME_FIRE3, 1 ); break;
		}
...
{
	int fl;
	fl = gEngfuncs.pEventAPI->EV_FindModelIndex( "sprites/playerflame.spr" );
	gEngfuncs.pEfxAPI->R_FireField( args->origin, 48, fl, 32, 0, 0.25);
}

void EV_FlareBurst( struct event_args_s *args )
{
	gEngfuncs.pEfxAPI->R_ParticleExplosion2( args->origin , 111, 8 );
}

int		ramp3[8] = {0x6d, 0x6b, 6, 5, 4, 3};

void EV_QRocketTrailCallback ( struct tempent_s *ent, float frametime, float currenttime )
{
...
void EV_QRocketTrail (event_args_t *args)
{
	...
	if ( pTrailSpawner != NULL)
	{
	   ...
	   pTrailSpawner->callback = EV_QRocketTrailCallback;
...
void EV_QExplosion (event_args_t *args)
{
	...
	gEngfuncs.pEfxAPI->R_Explosion( origin, modelIndex, 2.5, 15, TE_EXPLFLAG_NOSOUND );

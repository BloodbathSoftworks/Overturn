...
// 'links' the entity into the world
void UTIL_SetOrigin( entvars_t *ent, const Vector &vecOrigin )
{
	SET_ORIGIN(ENT(ent), vecOrigin );
}
...
void UTIL_CustomMessage( CBaseEntity *pEntity, int type, const char *pMessage, const char *pMessage2 )
{
   if ( !pEntity || !pEntity->IsNetClient() )
      return;
...
void UTIL_ExplosionSound ( Vector origin )
{
	switch(RANDOM_LONG(0, 2))
	{
	case 0: UTIL_EmitAmbientSound(ENT(0), origin, "weapons/explode3.wav", 1, 0.3, 0, 100); break;
	case 1: UTIL_EmitAmbientSound(ENT(0), origin, "weapons/explode4.wav", 1, 0.3, 0, 100); break;
	case 2: UTIL_EmitAmbientSound(ENT(0), origin, "weapons/explode5.wav", 1, 0.3, 0, 100); break;
	}
}

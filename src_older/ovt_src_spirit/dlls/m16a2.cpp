...
void CM16A2::PrimaryAttack()
{
	...
	UTIL_ScreenShake( m_pPlayer->GetGunPosition( ), 5.0, 30.0, 0.15, 0.0 );
  ...
  int flags;
	...
	m_pPlayer->m_flNextAttack = m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.1;
...
void CM16A2::SecondaryAttack()
{
	...
	{
		// optimized multiplayer. Widened to make it easier to hit a moving player
		vecDir = m_pPlayer->FireBulletsPlayer( 3, vecSrc, vecAiming, VECTOR_CONE_4DEGREES, 8192, BULLET_PLAYER_MP5, 2, 0, m_pPlayer->pev, m_pPlayer->random_seed );
	}
	else
	{
		// single player spread
		vecDir = m_pPlayer->FireBulletsPlayer( 3, vecSrc, vecAiming, VECTOR_CONE_2DEGREES, 8192, BULLET_PLAYER_MP5, 2, 0, m_pPlayer->pev, m_pPlayer->random_seed );
	}

...
//			m_iShards = atof(pkvd->szValue);
	...
	else if (FStrEq(pkvd->szKeyName, "spawnobject") )
	{
		...
		if ( object > 0 && object < ARRAYSIZE_HL(pSpawnObjects) )
...
const char **CBreakable::MaterialSoundList( Materials precacheMaterial, int &soundCount )
{
	...
	case matWood:
		...
		soundCount = ARRAYSIZE_HL(pSoundsWood);
		break;
	case matFlesh:
		pSoundList = pSoundsFlesh;
		soundCount = ARRAYSIZE_HL(pSoundsFlesh);
		break;
	case matComputer:
	case matUnbreakableGlass:
	case matGlass:
		pSoundList = pSoundsGlass;
		soundCount = ARRAYSIZE_HL(pSoundsGlass);
		break;

	case matMetal:
		pSoundList = pSoundsMetal;
		soundCount = ARRAYSIZE_HL(pSoundsMetal);
		break;

	case matCinderBlock:
	case matRocks:
		pSoundList = pSoundsConcrete;
		soundCount = ARRAYSIZE_HL(pSoundsConcrete);
		break;
...
void CBreakable::MaterialSoundRandom( edict_t *pEdict, Materials soundMaterial, float volume )
{
	...
	if ( soundCount )
		EMIT_SOUND_DYN( pEdict, CHAN_BODY, pSoundList[ RANDOM_LONG(0,soundCount-1) ], volume, 1.0, SND_AMBIENT, 100 );
}
...
//		return;
	...
	if (i)
		EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, rgpsz[RANDOM_LONG(0,i-1)], fvol, ATTN_NORM, SND_AMBIENT, pitch);
}
	...
	if (RANDOM_LONG(0,1) )
	{
		switch( m_Material )
		{
			case matComputer:
			{
				...
				{
					case 0: EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "buttons/spark1.wav", flVolume, ATTN_NORM, SND_AMBIENT, 100);	break;
					case 1: EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "buttons/spark2.wav", flVolume, ATTN_NORM, SND_AMBIENT, 100);	break;
					case 2: EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "buttons/spark3.wav", flVolume, ATTN_NORM, SND_AMBIENT, 100);	break;
					case 3: EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "buttons/spark4.wav", flVolume, ATTN_NORM, SND_AMBIENT, 100);	break;
					case 4: EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "buttons/spark5.wav", flVolume, ATTN_NORM, SND_AMBIENT, 100);	break;
					case 5: EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "buttons/spark6.wav", flVolume, ATTN_NORM, SND_AMBIENT, 100);	break;
				}
			}
			break;
...
void CBreakable::Die( void )
{
	...
	case matGlass:
		...
		{
		case 0:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustglass1.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		case 1:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustglass2.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		case 2:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustglass3.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		}
		cFlag = BREAK_GLASS;
		break;
	...
	case matWood:
		switch ( RANDOM_LONG(0,1) )
		{
		case 0:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustcrate1.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		case 1:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustcrate2.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		case 2:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustcrate3.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		}
		cFlag = BREAK_WOOD;
		break;
	...
	case matMetal:
		switch ( RANDOM_LONG(0,1) )
		{
		case 0:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustmetal1.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		case 1:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustmetal2.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		}
		cFlag = BREAK_METAL;
		break;
	...
	case matFlesh:
		switch ( RANDOM_LONG(0,1) )
		{
		case 0:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustflesh1.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		case 1:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustflesh2.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		}
		cFlag = BREAK_FLESH;
		break;
	...
	case matCinderBlock:
		switch ( RANDOM_LONG(0,1) )
		{
		case 0:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustconcrete1.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		case 1:	EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustconcrete2.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);	
			break;
		}
	...
	case matCeilingTile:
		EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "debris/bustceiling.wav", fvol, ATTN_NORM, SND_AMBIENT, pitch);
		break;
	}

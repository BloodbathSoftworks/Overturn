...
int CGlobalState::Save( CSave &save )
{
	...
	if ( !save.WriteFields( "cGLOBAL", "GLOBAL", this, m_SaveData, ARRAYSIZE_HL(m_SaveData) ) )
		return 0;
	
	pEntity = m_pList;
	for ( i = 0; i < m_listCount && pEntity; i++ )
	{
		if ( !save.WriteFields( "cGENT", "GENT", pEntity, gGlobalEntitySaveData, ARRAYSIZE_HL(gGlobalEntitySaveData) ) )
			return 0;
...
int CGlobalState::Restore( CRestore &restore )
{
	...
	ClearStates();
	if ( !restore.ReadFields( "GLOBAL", this, m_SaveData, ARRAYSIZE_HL(m_SaveData) ) )
		return 0;
	...
	{
		if ( !restore.ReadFields( "GENT", &tmpEntity, gGlobalEntitySaveData, ARRAYSIZE_HL(gGlobalEntitySaveData) ) )
			return 0;
...
// sounds used from C physics code
	...
	PRECACHE_SOUND( "items/wpnrespawn.wav" );//!!! temporary sound for respawning weapons.
	PRECACHE_SOUND( "items/gunpickup1.wav" );// player picks up a gun.
...
//
	...
	for (i = 0; i < ARRAYSIZE_HL(gDecals); i++ )
...
//LRC- ends
	...
	//JIW
	else if ( FStrEq(pkvd->szKeyName, "startroomtype") )
	{
		pev->iuser1 =atof(pkvd->szValue);
		pkvd->fHandled = TRUE;
		ALERT(at_console,"CWorld start roomtype %d\n", pev->iuser1);
	}
	//JIW
	else
		CBaseEntity::KeyValue( pkvd );

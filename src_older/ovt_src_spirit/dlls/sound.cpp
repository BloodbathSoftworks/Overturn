...
void CAmbientGeneric :: SendInitMessage( CBasePlayer *player )
{
	...
	if(player->m_bHadFirstSpawn)
		return;

	if (m_fActive)
	{
		MESSAGE_BEGIN(MSG_ONE, gmsgEmitASound, NULL, player->pev);
			WRITE_STRING(STRING(pev->message));
			WRITE_SHORT(ENTINDEX(edict()));
			WRITE_COORD(m_dpv.vol);
			WRITE_COORD(m_flAttenuation*100);
			WRITE_SHORT(NULL);
			WRITE_BYTE(m_dpv.pitch);
...
void CEnvSound :: Think( void )
{
	...
	if (FEnvSoundInRange(pev, VARS(pentPlayer), &flRange)) 
	{
		if (flRange < pPlayer->m_flSndRange || pPlayer->m_flSndRange == 0) 
		{
			...
			MESSAGE_BEGIN( MSG_ONE, gmsgRoomType, NULL, pPlayer->edict() );		// use the magic #1 for "one client"
				WRITE_SHORT( (short)m_flRoomtype );					// sequence number
			MESSAGE_END();

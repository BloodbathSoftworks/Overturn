...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] = 
{
	...
	DEFINE_FIELD( CBasePlayer, medkits, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, batteries, FIELD_INTEGER ),
	...
	//FMOD SOUND ENGINE
	DEFINE_FIELD( CBasePlayer, m_iCurrentRoomType, FIELD_INTEGER ),
	//FMOD SOUND ENGINE
};	
...
int gmsgEmitSound		= 0; 
int gmsgEmitASound		= 0; 
int gmsgSEPrecache		= 0; 
int gmsgRoomType		= 0; 
int gmsgPlayAudio		= 0; 
...
void LinkUserMessages( void )
{
	...
	gmsgTABMenu = REG_USER_MSG("TABMenu", 0);
	gmsgMedkit = REG_USER_MSG("Medkit", 1);
	gmsgBatteries = REG_USER_MSG("Batteries", 1);
	gmsgLongJump = REG_USER_MSG("LongJump", 1);

	gmsgStatusText = REG_USER_MSG("StatusText", -1);
	gmsgStatusValue = REG_USER_MSG("StatusValue", 3); 

	gmsgEmitSound = REG_USER_MSG("EmitSound", -1); 
	gmsgEmitASound = REG_USER_MSG("EmitASound", -1); 
	gmsgSEPrecache = REG_USER_MSG("SEPrecache", 0);
	gmsgRoomType = REG_USER_MSG("RoomType", 2); 
	gmsgPlayAudio = REG_USER_MSG("PlayAudio", -1);
}
...
//JIW - player voice sounds
void CBasePlayer :: PlayerPainSound( int bitsDamageType )
{
	if(bitsDamageType & DMG_FALL)
		return;

	if (pev->waterlevel == 3)
		SENTENCEG_PlayRndSz(ENT(pev), "PL_DROWN", 0.8, ATTN_NORM, 0, RANDOM_LONG(96,98));
	else if(bitsDamageType & DMG_NERVEGAS)
		SENTENCEG_PlayRndSz(ENT(pev), "PL_GASP", 0.8, ATTN_NORM, 0, RANDOM_LONG(96,98));
	else if(bitsDamageType & (DMG_BURN | DMG_POISON))
		SENTENCEG_PlayRndSz(ENT(pev), "PL_YELP", 0.8, ATTN_NORM, 0, RANDOM_LONG(96,98));
	else
		SENTENCEG_PlayRndSz(ENT(pev), "PL_PAIN", 0.8, ATTN_NORM, 0, RANDOM_LONG(96,98));
}
void CBasePlayer :: PlayerDeathSound( BOOL bGibbed )
{
	if(bGibbed)
		SENTENCEG_PlayRndSz(ENT(pev), "PL_GIBDIE", 0.8, ATTN_NORM, 0, RANDOM_LONG(98,100));
	else if (pev->waterlevel == 3)
		SENTENCEG_PlayRndSz(ENT(pev), "PL_WTRDIE", 0.8, ATTN_NORM, 0, RANDOM_LONG(98,100));
	else
		SENTENCEG_PlayRndSz(ENT(pev), "PL_DEATH", 0.8, ATTN_NORM, 0, RANDOM_LONG(98,100));
}//JIW
...
void CBasePlayer :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	if ( pev->takedamage )
	{
		...
		case HITGROUP_HEAD:
			...
			SpawnBlood(ptr->vecEndPos, BloodColor(), flDamage * 44);//JIW - MOAR BLOOD
			break;
		case HITGROUP_CHEST:
...
//	int ftrivial;
	...
	//JIW - pain sound
	if ( pev->deadflag == DEAD_NO && m_flNextPainTime <= gpGlobals->time && flDamage >= 2 )
	{
		PlayerPainSound( m_bitsLastDamageType );
		m_flNextPainTime = gpGlobals->time + 0.25;//RANDOM_FLOAT(0.5,1.0);//dont make pain sound for 0.5 - 1 second
	}
	//JIW

	//JIW
	if (bitsDamageType & DMG_IGNITE)// && pev->waterlevel < 2)
	{
		if(pAttacker && pAttacker != this)
			m_pFireAttacker = pAttacker;
		else
			m_pFireAttacker = NULL;

		m_flBurnTime = gpGlobals->time + sv_burntime.value;
		m_flNextBurnTime = gpGlobals->time;
	}
	//JIW
...
//LRC
void CBasePlayer::RemoveItems( int iWeaponMask, int i9mm, int i10mm, int i357, int i12Buck, int i20Buck, int iBolt, int iM203Gren, int iRock, int iUranium, int iPolonium, int iSatchel, int iSnark, int iTrip, int iGren, int iHornet )
{
	int i;
	...
	RemoveAmmo("9mm", i9mm);
	RemoveAmmo("10mm", i10mm);
	RemoveAmmo("357", i357);
	RemoveAmmo("12buckshot", i12Buck);
	RemoveAmmo("20buckshot", i20Buck);
	RemoveAmmo("bolts", iBolt);
	RemoveAmmo("m203grenades", iM203Gren);
	RemoveAmmo("uranium", iUranium);
	RemoveAmmo("polonium", iPolonium);
	...
	for (i = 0; i < MAX_ITEM_TYPES; i++)
	{
		...
		while (pCurrentItem->m_pNext)
		{
			...
			{
				((CBasePlayerWeapon*)pCurrentItem)->DrainClip(this, FALSE, i9mm, i10mm, i357, i12Buck, i20Buck, iBolt, iM203Gren, iRock, iUranium, iPolonium, iSatchel, iSnark, iTrip, iGren);
...
//				ALERT(at_console, "Removing %s. (id = %d)\n", pCurrentItem->m_pNext->pszName(), pCurrentItem->m_pNext->m_iId);
			...
			else
			{
				...
				((CBasePlayerWeapon*)pCurrentItem)->DrainClip(this, TRUE, i9mm, i10mm, i357, i12Buck, i20Buck, iBolt, iM203Gren, iRock, iUranium, iPolonium, iSatchel, iSnark, iTrip, iGren);
...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	g_iInZoom = 0;//JIW - zoom/FOV stuff

	// reset FOV
	pev->fov = m_iFOV = m_iClientFOV = 0;

	m_flBurnTime = m_flNextBurnTime = -1;//XDM
	m_pFireAttacker = NULL;
	...
	if ( ( pev->health < -40 && iGib != GIB_NEVER ) || iGib == GIB_ALWAYS )
	{
		...
		PlayerDeathSound(TRUE);//JIW - new death sounds - play even if gibbed too	
		return;
	}

	PlayerDeathSound(FALSE);//JIW
//	DeathSound();
...
void CBasePlayer::TabulateAmmo()
{
	...
	ammo_10mm = AmmoInventory( GetAmmoIndex( "10mm" ) );
	ammo_357 = AmmoInventory( GetAmmoIndex( "357" ) );
	ammo_m203grenades = AmmoInventory( GetAmmoIndex( "m203grenades" ) );
	ammo_bolts = AmmoInventory( GetAmmoIndex( "bolts" ) );
	ammo_12buckshot = AmmoInventory( GetAmmoIndex( "12buckshot" ) );
	ammo_20buckshot = AmmoInventory( GetAmmoIndex( "20buckshot" ) );
	ammo_rockets = AmmoInventory( GetAmmoIndex( "rockets" ) );
	ammo_uranium = AmmoInventory( GetAmmoIndex( "uranium" ) );
	ammo_polonium = AmmoInventory( GetAmmoIndex( "polonium" ) );
...
void CBasePlayer::WaterMove()
{
	...
	if (!RANDOM_LONG(0,0x1f) && RANDOM_LONG(0,AIRTIME-1) >= air)
	{
		...
		{
			case 0:	EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim1.wav", 0.4, ATTN_NORM); break;
			case 1:	EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim2.wav", 0.4, ATTN_NORM); break;
			case 2:	EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim3.wav", 0.4, ATTN_NORM); break;
			case 3:	EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim4.wav", 0.4, ATTN_NORM); break;
		}
	}
...
void CBasePlayer::ShowTABMenu()
{
    MESSAGE_BEGIN(MSG_ONE, gmsgTABMenu, NULL, pev);
	MESSAGE_END();
}

int CBasePlayer::UseMedkit()
{
	if (pev->health == 100)
	{
		EMIT_SOUND( ENT(pev), CHAN_ITEM, "common/wpn_denyselect.wav", 0.4, ATTN_NORM);
	...
	m_flBurnTime = m_flNextBurnTime = -1;//XDM
	EMIT_SOUND( ENT(pev), CHAN_ITEM, "items/medshot4.wav", 0.4, ATTN_NORM);
...
int CBasePlayer::UseBattery()
{
	...
	pev->armorvalue += gSkillData.batteryCapacity;
	if(pev->armorvalue > 100) pev->armorvalue = 100;
...
// many features in this function use v_forward, so makevectors now.
	...
	/*switch ( (int)(RANDOM_FLOAT(0,1) * 5) )
	{
		case 0: EMIT_SOUND( ENT(pev), CHAN_VOICE, "player/jumpshort02.wav", 1, ATTN_NORM );	break;
		case 1: EMIT_SOUND( ENT(pev), CHAN_VOICE, "player/jumpshort07.wav", 1, ATTN_NORM );	break;
		case 2: EMIT_SOUND( ENT(pev), CHAN_VOICE, "player/jumpshort16.wav", 1, ATTN_NORM );	break;
		case 3: EMIT_SOUND( ENT(pev), CHAN_VOICE, "player/jumpshort19.wav", 1, ATTN_NORM );	break;
		case 4: EMIT_SOUND( ENT(pev), CHAN_VOICE, "player/jumpshort29.wav", 1, ATTN_NORM );	break;
	}*/
	SENTENCEG_PlayRndSz(ENT(pev), "PL_JUMP", 0.8, ATTN_NORM, 0, RANDOM_LONG(96,98));
...
//	CheckDesiredList( ); //LRC
	...
	if (m_flBurnTime != -1 && m_flNextBurnTime != -1 && m_flNextBurnTime <= gpGlobals->time)
	{
		PLAYBACK_EVENT_FULL( 0, edict(), m_usFireField, 0, (float *)&pev->origin, (float *)&g_vecZero, 0, 0, 0, 0, 0, 0 );
		m_flNextBurnTime = gpGlobals->time + 0.75;

		if(m_pFireAttacker)
			TakeDamage(m_pFireAttacker->pev,m_pFireAttacker->pev,5,DMG_BURN);
		else
...
// dont let uninitialized value here hurt the player
	...
	if(pWorldSpawn && pWorldSpawn->v.iuser1 > 0)//default roomtype
		m_iCurrentRoomType = pWorldSpawn->v.iuser1; 
	else
		m_iCurrentRoomType = 0;
	m_bUpdateRoomType = TRUE;
	//ALERT(at_console,"Changed room type to %d\n", m_iCurrentRoomType);
	//FMOD SOUND ENGINE
...
void CBasePlayer :: Precache( void )
{
	...
	m_bUpdateRoomType = TRUE;//UPDATE
	m_bHadFirstSpawn = FALSE;
	m_bClientFogRefresh = TRUE;
	//FMOD SOUND ENGINE
}
...
int CBasePlayer::Save( CSave &save )
{
	...
	return save.WriteFields( "cPLAYER", "PLAYER", this, m_playerSaveData, ARRAYSIZE_HL(m_playerSaveData) );
}
...
int CBasePlayer::Restore( CRestore &restore )
{
	...
	int status = restore.ReadFields( "PLAYER", this, m_playerSaveData, ARRAYSIZE_HL(m_playerSaveData) );
...
BOOL CBasePlayer :: FlashlightIsOn( void )				//Tackle this area.
{
#ifdef XENWARRIOR
...
const int Takt = 2000;
	...
	case 101:
		...
		GiveNamedItem( "ammo_mp5" );
		GiveNamedItem( "ammo_m203grenades" );
		...
		GiveNamedItem( "ammo_12buckshot" );
		GiveNamedItem( "ammo_20buckshot" );
...
void CBasePlayer::ItemPostFrame()
{
	...
	// Always have fists by
	//if(!(pev->weapons & (1<<WEAPON_POWERGLOVE)))
	//{
	//	edict_t	*pent = CREATE_NAMED_ENTITY(MAKE_STRING("weapon_powerglove"));
	//	if ( FNullEnt( pent ) )
	//	{
	//		ALERT ( at_debug, "NULL Ent in GiveNamedItem!\n" );
	//		return;
...
=========================================================
*/
extern void SE_RunPrecache( CBaseEntity *pplayer );
void CBasePlayer :: UpdateClientData( void )
{
	...
	if(m_iClientBatteries != batteries)
	{
		m_iClientBatteries = batteries;
		MESSAGE_BEGIN(MSG_ONE, gmsgBatteries, NULL, pev);
			WRITE_BYTE( batteries );
		MESSAGE_END();
	}

	if(m_iClientHealthkits != medkits)
	{
		m_iClientHealthkits = medkits;
		MESSAGE_BEGIN(MSG_ONE, gmsgMedkit, NULL, pev);
			WRITE_BYTE( medkits );
		MESSAGE_END();
	}

	if( m_bClientFogRefresh )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgSetFog, NULL, this->pev );
			WRITE_BYTE ( m_vecFogColor.x );
			WRITE_BYTE ( m_vecFogColor.y );
			WRITE_BYTE ( m_vecFogColor.z );
			WRITE_SHORT ( NULL );
			WRITE_SHORT ( m_iFogStartDist );
			WRITE_SHORT ( m_iFogEndDist );
		MESSAGE_END();

		m_bClientFogRefresh = FALSE;
	}
	...
	//SOUND ENGINE
	if ( m_bUpdateRoomType )
	{
		pev->vuser1[2] = m_iCurrentRoomType;
		m_bUpdateRoomType = FALSE;
	}
	//SOUND ENGINE
...
TYPEDESCRIPTION	CStripWeapons::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CStripWeapons, m_i10mm, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_i357, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_i12Buck, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_i20Buck, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iBolt, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iM203Gren, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iRock, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iUranium, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iPolonium, FIELD_INTEGER ),
...
void CStripWeapons :: KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "12buckshot"))
	{
		m_i12Buck = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "20buckshot"))
	{
		m_i20Buck = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	...
	else if (FStrEq(pkvd->szKeyName, "m203grenades"))
	{
		m_iM203Gren = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	...
	else if (FStrEq(pkvd->szKeyName, "polonium"))
	{
		m_iPolonium = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
...
void CStripWeapons :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pPlayer )
	{
		pPlayer->RemoveItems( pev->spawnflags, m_i9mm, m_i10mm, m_i357, m_i12Buck, m_i20Buck, m_iBolt, m_iM203Gren, m_iRock, m_iUranium, m_iPolonium, m_iSatchel, m_iSnark, m_iTrip, m_iGren, m_iHornet);

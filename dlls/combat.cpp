...
#define	HUMAN_GIB_COUNT			11//JIW//6
...
//	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "common/bodysplat.wav", 1, ATTN_NORM);		
	UTIL_EmitAmbientSound(ENT(0), pev->origin, "common/bodysplat.wav", 1, ATTN_NORM, 0, 100);
	...
	if ( iszCustomGibs = HasCustomGibs() ) //LRC - monster_generic can have a custom gibset
	{
		if ( CVAR_GET_FLOAT("violence_hgibs") != 0 )
		{
			...
			CGib::SpawnRandomGibs( pev, 10, 1, STRING(iszCustomGibs) );//JIW//4
	...
	else if ( HasHumanGibs() )
	{
		if ( CVAR_GET_FLOAT("violence_hgibs") != 0 )// Only the player will ever fail this test
		{
			...
			CGib::SpawnRandomGibs( pev, 12, 1 );	// throw some human gibs.
		}
		gibbed = TRUE;
	}
...
void CGib :: BounceGibTouch ( CBaseEntity *pOther )
{
    ...
    switch ( RANDOM_LONG( 0, 5 ) )//JIW
    {
        case 0:EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/flesh1.wav", 0.7, ATTN_NORM);break;
		case 1:EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/flesh2.wav", 0.7, ATTN_NORM);break;
		case 2:EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/flesh3.wav", 0.7, ATTN_NORM);break;
	//	case 3:EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/flesh4.wav", 0.7, ATTN_NORM);break;
		case 3:EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/flesh5.wav", 0.7, ATTN_NORM);break;
		case 4:EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/flesh6.wav", 0.7, ATTN_NORM);break;
		case 5:EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/flesh7.wav", 0.7, ATTN_NORM);break;
    }
...
int CBaseMonster :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( !IsPlayer() && /*IsAlive()*/(pev->deadflag == DEAD_NO && pev->health > 0) && m_flNextPainTime <= gpGlobals->time && flDamage >= 2 )//JIW
	{
		// no pain sound during death animation.
		PainSound();// "Ouch!"
		m_flNextPainTime = gpGlobals->time + RANDOM_FLOAT(0.5,1.0);//JIW - dont make pain sound for 0.5 - 1 second
	}
	...
	//JIW - reset and send last damage type
	m_bitsLastDamageType = 0;
	m_bitsLastDamageType |= bitsDamageType;
	...
	for (ULONG iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			case BULLET_MONSTER_762:		
				pEntity->TraceAttack(pevAttacker, gSkillData.monDmgM40a1, vecDir, &tr, DMG_BULLET); 
				DecalGunshot( &tr, iBulletType );
				break;
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			case BULLET_PLAYER_12BUCKSHOT:	
				 // make distance based!
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmg12Buckshot, vecDir, &tr, DMG_BULLET); 
				break;
			
			case BULLET_PLAYER_20BUCKSHOT:	
				 // make distance based!
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmg20Buckshot, vecDir, &tr, DMG_BULLET); 
				break;
			...
			case BULLET_PLAYER_762:		
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgM40a1, vecDir, &tr, DMG_BULLET); 
				break;

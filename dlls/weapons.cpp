...
DLL_GLOBAL unsigned short m_usFireField;//JIW
DLL_GLOBAL unsigned short m_usFlareBurst;//JIW
DLL_GLOBAL unsigned short m_usQRocketTrail;//JIW
DLL_GLOBAL unsigned short m_usQExplosion;//JIW
...
void W_Precache(void)
{
	...
	// shotgun
	UTIL_PrecacheOtherWeapon( "weapon_powerglove" );
	UTIL_PrecacheOtherWeapon( "weapon_shotgun" );
	UTIL_PrecacheOtherWeapon( "weapon_stakeout" );
	UTIL_PrecacheOther( "ammo_buckshot" );
	UTIL_PrecacheOther( "ammo_20buckshot" );
	UTIL_PrecacheOther( "ammo_12buckshot" );
	...
	UTIL_PrecacheOther( "ammo_m203grenades" );
#endif
	...
	UTIL_PrecacheOther( "ammo_canister" );
...
#endif
	UTIL_PrecacheOtherWeapon( "weapon_m16a2" );
	...
	m_usFireField = PRECACHE_EVENT( 1, "events/firefield.sc" ); // buz
	m_usFlareBurst = PRECACHE_EVENT( 1, "events/flareburst.sc" ); // buz
	m_usQRocketTrail = PRECACHE_EVENT( 1, "events/qrockettrail.sc" ); // buz
	m_usQExplosion = PRECACHE_EVENT( 1, "events/qexplosion.sc" ); // buz
...
void CBasePlayerItem::Materialize( void )
{
	if ( pev->effects & EF_NODRAW )
	{
		...
		EMIT_SOUND_DYN( ENT(pev), CHAN_WEAPON, "items/wpnrespawn.wav", 1, ATTN_NORM, 0, 150 );
...
/*void CBasePlayerItem::DefaultUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	//}
		...
		pCaller->AddPlayerItem( this );
		AttachToPlayer( pPlayer );
		EMIT_SOUND(ENT(pPlayer->pev), CHAN_ITEM, "items/gunpickup2.wav", 1, ATTN_NORM);

	//SUB_UseTargets( pCaller, USE_TOGGLE, 0 ); // UNDONE: when should this happen?
}*/
...
int CBasePlayerWeapon::AddToPlayer( CBasePlayer *pPlayer )
{
	if(m_iId != WEAPON_POWERGLOVE)
	{
		if ((!(pPlayer->m_afButtonPressed & IN_USE) && ( !g_pGameRules->IsMultiplayer() )) || (gEvilImpulse101))
		{

			if (MessageDelay < gpGlobals->time)
			{
				UTIL_CustomMessage( pPlayer, MSG_PICKUP, "", STRING(pev->classname));
				MessageDelay = gpGlobals->time + 10;
			}
			return FALSE;
		}
	}
...
BOOL CBasePlayerWeapon :: AddSecondaryAmmo( int iCount, char *szName, int iMax )
{
	...
	if (iIdAmmo > 0)
	{
		...
		EMIT_AMBIENT_SOUND(ENT(pev), pev->origin, "items/ammopickup2.wav", 1, ATTN_NORM, 0, PITCH_NORM);
	}
	return iIdAmmo > 0 ? TRUE : FALSE;
}
...
BOOL CBasePlayerWeapon :: DefaultDeploy( char *szViewModel, char *szWeaponModel, int iAnim, char *szAnimExt, int skiplocal /* = 0 */, int body )
{
	...
	if(!szWeaponModel) m_pPlayer->pev->weaponmodel = NULL;
	else m_pPlayer->pev->weaponmodel = MAKE_STRING(szWeaponModel);
...
float CBasePlayerWeapon :: GetSequenceTime( int iAnim )
{
	pev->modelindex = MODEL_INDEX(STRING(m_pPlayer->pev->viewmodel));
	...
	mstudioseqdesc_t *pseqdesc = (mstudioseqdesc_t *)((byte *)pHdr + pHdr->seqindex)+iAnim;
...
void CBasePlayerWeapon::SpinWeapon( void )
{
}
...
//=========================================================
void CBasePlayerWeapon::DrainClip(CBasePlayer* pPlayer, BOOL keep, int i9mm, int i10mm, int i357, int i12Buck, int i20Buck, int iBolt, int iM203Gren, int iRock, int iUranium, int iPolonium, int iSatchel, int iSnark, int iTrip, int iGren )
{
	int iPAI = PrimaryAmmoIndex();
	int iAmt;
	if (iPAI == -1) return;
	else if (iPAI == pPlayer->GetAmmoIndex("9mm"))			iAmt = i9mm;
	else if (iPAI == pPlayer->GetAmmoIndex("10mm"))			iAmt = i10mm;
	else if (iPAI == pPlayer->GetAmmoIndex("357"))			iAmt = i357;
	else if (iPAI == pPlayer->GetAmmoIndex("12buckshot"))	iAmt = i12Buck;
	else if (iPAI == pPlayer->GetAmmoIndex("20buckshot"))	iAmt = i20Buck;
	else if (iPAI == pPlayer->GetAmmoIndex("bolts"))		iAmt = iBolt;
	else if (iPAI == pPlayer->GetAmmoIndex("m203grenades"))	iAmt = iM203Gren;
	else if (iPAI == pPlayer->GetAmmoIndex("uranium"))		iAmt = iUranium;
	else if (iPAI == pPlayer->GetAmmoIndex("polonium"))		iAmt = iPolonium;
...
TYPEDESCRIPTION	CStakeout::m_SaveData[] = 
{
	DEFINE_FIELD( CStakeout, m_flNextReload, FIELD_TIME ),
	DEFINE_FIELD( CStakeout, m_fInSpecialReload, FIELD_INTEGER ),
	DEFINE_FIELD( CStakeout, m_flNextReload, FIELD_TIME ),
	// DEFINE_FIELD( CStakeout, m_iShell, FIELD_INTEGER ),
	DEFINE_FIELD( CStakeout, m_flPumpTime, FIELD_TIME ),
};
IMPLEMENT_SAVERESTORE( CStakeout, CBasePlayerWeapon );
#ifdef PARTICLE_FUSION
...
#endif
#ifdef PARTICLEFUSION
TYPEDESCRIPTION	CDeagle::m_SaveData[] = 
{
	DEFINE_FIELD( CDeagle, m_fEagleLaserActive, FIELD_INTEGER ),
};
...
{
	DEFINE_FIELD( CPython, m_fLaserActive, FIELD_INTEGER ),
};

...
int __MsgFunc_TABMenu(const char *pszName, int iSize, void *pbuf)
{
	if (gViewPort)
		return gViewPort->MsgFunc_TABMenu( pszName, iSize, pbuf );
	return 0;
}
...
int __MsgFunc_EmitSound( const char *pszName, int iSize, void *pBuf )
{
	gSoundSystem.MsgFunc_EmitSound(pszName, iSize, pBuf);
	return 1;
}

int __MsgFunc_EmitASound( const char *pszName, int iSize, void *pBuf )
{
	gSoundSystem.MsgFunc_EmitASound(pszName, iSize, pBuf);
	return 1;
}

int __MsgFunc_PlayAudio( const char *pszName, int iSize, void *pBuf )
{
	gSoundSystem.MsgFunc_PlayAudio(pszName, iSize, pBuf);
	return 1;
}

/*int __MsgFunc_RoomType( const char *pszName, int iSize, void *pBuf )
{
	gSoundSystem.MsgFunc_RoomType(pszName, iSize, pBuf);
	return 1;
}*/

int __MsgFunc_SEPrecache( const char *pszName, int iSize, void *pBuf )
{
	gSoundSystem.MsgFunc_Precache(pszName, iSize, pBuf);
	return 1;
}
...
void CHud :: Init( void )
{
	...
	HOOK_MESSAGE( TABMenu );

	HOOK_MESSAGE( Spectator );
	HOOK_MESSAGE( AllowSpec );

	HOOK_MESSAGE( EmitSound );
	HOOK_MESSAGE( EmitASound );
//	HOOK_MESSAGE( RoomType );
	HOOK_MESSAGE( PlayAudio );
	HOOK_MESSAGE( SEPrecache );

	// Model Projected Shadows
	CVAR_CREATE("r_shadows", "1", FCVAR_ARCHIVE );
	...
	m_iLogo = 0;
	//m_iFOV = 0;
	m_iHUDColor = 0xF70000; //255,160,0 -- LRC
	...
	m_pCvarViewSway = gEngfuncs.pfnRegisterVariable ( "cl_viewsway", "0", FCVAR_CLIENTDLL|FCVAR_ARCHIVE );

	m_flFOV = default_fov->value;//JIW

...
//JIW - playlist
#define MAX_PL_ENTRIES 64
char	g_szPlayList[MAX_PL_ENTRIES][MAX_PATH];
int g_iplPos = 0;
...
DWORD WINAPI WatchThread( LPVOID lpParam )
{
	...
	while(1)
	{
		...
		LONG nStyle = GetWindowLongPtr(pThreadData->hWindow, GWL_STYLE);
...
BOOL IsWON( void )
{
	...
	hDll = GetModuleHandleA("WONAuth.dll");
	if(hDll) return true;

	hDll = GetModuleHandleA("WONCrypt.dll");
	if(hDll) return true;

//	gEngfuncs.Con_DPrintf("Steam Version of Half-Life verified.\n");
	return false;
}
...
====================
*/
void __CmdFunc_StopAudio( void )
{
	if(!gSoundSystem.m_pMusicTrack.used)
		return;

	alSourceStop(gSoundSystem.m_pMusicTrack.source);
	alDeleteSources(1, &gSoundSystem.m_pMusicTrack.source);
	alDeleteBuffers(2, gSoundSystem.m_pMusicTrack.buffers);
	memset(&gSoundSystem.m_pMusicTrack, 0, sizeof(s_music_t));
}
...
void CSoundSystem :: Init ( void )
{
	...
	m_pCvarOcclusion = CVAR_CREATE("al_occlusion", "1", FCVAR_ARCHIVE | FCVAR_CLIENTDLL);

	gEngfuncs.pfnAddCommand("stopaudio", __CmdFunc_StopAudio);
	...
	{
		m_bWatchThread = true;
		HWND hWindow = FindWindow("Half-Life", NULL);
		if(!hWindow)
		{
			hWindow = FindWindow("Valve001", NULL);
			if(!hWindow)
			{
				m_bWatchThread = false;
			}
		}
		...
		if(m_bWatchThread)
		{
			...
			sThreadData.hWindow = hWindow;

			DWORD dwThreadID;
			g_hThreadHandle = CreateThread(NULL, 0, WatchThread, &sThreadData, 0, &dwThreadID);
			SetThreadPriority(g_hThreadHandle, THREAD_PRIORITY_LOWEST);
		}
	}
}
...
void CSoundSystem :: ResetEngine ( void )
{
	...
	memset(g_szPlayList, 0, sizeof(g_szPlayList));//JIW
	...
	if(m_iNumCached)
	{
		...
		{
			alDeleteBuffers(m_pCachedSounds[i].numbuffers, m_pCachedSounds[i].buffers);
			...
			if(m_pCachedSounds[i].buffers)
			{
				delete [] m_pCachedSounds[i].buffers;
			}
		}
	...
	// reset this
	memset(m_pMSGCache, 0, sizeof(m_pMSGCache));

	// Remove cache file
	char szfile[128];
	strcpy(szfile, gEngfuncs.pfnGetGameDirectory());
	strcat(szfile, "/");
	strcat(szfile, m_szLastLevelName);
	strcpy(&szfile[(strlen(szfile)-4)], "_soundlist.txt");

	if(!remove(szfile))
		gEngfuncs.Con_DPrintf("Cache file deleted successfully.\n");
...
void CSoundSystem::MsgFunc_Precache( const char *pszName, int iSize, void *pbuf )
{
	...
	byte *pfile = gEngfuncs.COM_LoadFile(szfile, 5, 0);
	if(pfile)
		return;

	char szcommand[64];
	strcpy(szcommand, "cmd dlfile ");
...
void CSoundSystem::W_HUDFrame( void )
{
	if(!m_bReset && m_bGotFrame)
	{
		if(!m_bGotRender)
		{
			gEngfuncs.Con_Printf("Resetting from W_HUDFrame\n");
			ResetEngine();
			return;
		}
		m_bGotRender = false;
...
void CSoundSystem::W_DNormal( void )
{
	...
	{
		gEngfuncs.Con_Printf("Resetting from W_DNormal\n");
		ResetEngine();
		return;
	}
	...
	if(m_pCvarDebug->value >= 1)
	{
		...
		for(int i = 0; i < MAX_ACTIVE_SOUNDS; i++)
		{
			...
			if(m_pActiveSounds[i].leafnum)
				glColor4f(0.0, 0.0, 1.0, 1.0);
			else
				glColor4f(1.0, 0.0, 0.0, 1.0);

			glBegin(GL_POINTS);
			glVertex3fv(m_pActiveSounds[i].origin+Vector(0, 0, 5));
			glEnd();
		}
...
void CSoundSystem::SetupEngine( void )
{
	...
	//
	// Copy current level name
	//
	strcpy(m_szLastLevelName, gEngfuncs.pfnGetLevelName());
	...
	if(pfile)
	{
		...
		gEngfuncs.Con_DPrintf("Cache file found.\n");
	...
	//JIW - load playlist!
	char szfullpath[256];
	sprintf(szfullpath, "%s/%s", gEngfuncs.pfnGetGameDirectory(), "sound/music/playlist.txt");
	FILE *pplaylist = fopen(szfullpath,"r");

	g_iplPos = 0;
	g_iPlayListNumItems = 0;
	if (pplaylist)
	{
		//I messily ripped this out of XDM's src because I'm a lazy fraud hack :B
		while (fgets(g_szPlayList[g_iPlayListNumItems], MAX_PATH, pplaylist) != NULL)
		{
			if (!strncmp(g_szPlayList[g_iPlayListNumItems], "//", 2) || g_szPlayList[0][g_iPlayListNumItems] == '\n')
			{
				continue;// skip comments or blank lines
			}

			char *ch = strchr(g_szPlayList[g_iPlayListNumItems], '\n');
			if (ch == NULL)// try windows-style
				strchr(g_szPlayList[g_iPlayListNumItems], '\r');
			...
			if (g_iPlayListNumItems >= MAX_PL_ENTRIES)
				break;
		}
		gEngfuncs.Con_Printf("Playlist loaded! %d total entries.\n",g_iPlayListNumItems);
		g_iPlayListNumItems-=1;//hacks
		g_iplPos = gEngfuncs.pfnRandomLong(0,g_iPlayListNumItems);//start from random track

		fclose(pplaylist);
	}
	//

	m_flUpdateMusic = gEngfuncs.GetClientTime() + 2.5;//JIW
}
...
s_active_t* CSoundSystem::AllocSound( const char *sample, int entindex, int flags, int channel )
{
	...
	if(entindex)
	{
		for(int i = 0; i < MAX_ACTIVE_SOUNDS; i++)
		{
			...
			if(m_pActiveSounds[i].flags & SND_AMBIENT)//JIW
				continue;
...
s_cache_t* CSoundSystem::PrecacheSound( char *sample )
{
	...
	while(1)
	{
		...
		if(!strncmp((const char*)pbegin, "cue ", 4))
		{
			...
			pcache->loopbegin = ByteToInt(pbegin+32)*(pcache->bitspersample>>3);
		}
	...
	{
		pcache->numbuffers = 2;
		pcache->buffers = new ALuint[pcache->numbuffers];

		alGenBuffers(pcache->numbuffers, pcache->buffers);
		alBufferData(pcache->buffers[0], format, pcache->data+pcache->dataoffset, pcache->loopbegin, pcache->samplerate);
		alBufferData(pcache->buffers[1], format, pcache->data+pcache->loopbegin, pcache->length-pcache->loopbegin+2, pcache->samplerate);
	}
	else
	{
		pcache->numbuffers = 1;
		...
		alGenBuffers(pcache->numbuffers, pcache->buffers);
		alBufferData(pcache->buffers[0], format, pcache->data+pcache->dataoffset, pcache->length, pcache->samplerate);
	}
...
void CSoundSystem::CalcRefDef( ref_params_t *pparams )
{
	...
	//
	//JIW
	if(m_iActiveReverb != g_iRoomType)
		m_iActiveReverb = g_iRoomType;
	...
	for(int i = 0; i < MAX_ACTIVE_SOUNDS; i++)
	{
		...
		if(flframetime)
		{
			...
			{
				endpos = endpos*((float)psound->chunk->end/100.0f);
			}
			...
			if(psound->datapos >= endpos)
			{
				...
				{
					psound->datapos = psound->cache->loopbegin;
				}
				else
				{
					if(psound->chunk && psound->chunk->next)
					{
						...
						psound->pitch = psound->mainpitch*((float)psound->chunk->pitch/100.0f);
						psound->volume = psound->volume*((float)psound->chunk->volume/100.0f);
						psound->datapos = pcache->length*((float)psound->chunk->start/100.0f);
		...
		if(psound->entity && !(psound->flags & SND_2D) && psound->entity->curstate.messagenum == imsgnum)
		{
			...
			if(pmodel->type == mod_brush)// && !(psound->entity->curstate.effects & FL_SOUNDORG))
			{	
			...
			else if(pmodel->type == mod_studio)
			{
				...
				studiohdr_t *phdr = (studiohdr_t *)IEngineStudio.Mod_Extradata(psound->entity->model);
				if(psound->entity->curstate.sequence >= phdr->numseq)
				{
					psound->entity->curstate.sequence = phdr->numseq;
				}
	
				mstudioseqdesc_t *pseqdesc = (mstudioseqdesc_t *)((byte *)phdr + phdr->seqindex)+psound->entity->curstate.sequence;
				vec3_t vmins = psound->entity->origin + pseqdesc->bbmin;
				vec3_t vmaxs = psound->entity->origin + pseqdesc->bbmax;
				VectorScale((vmins+vmaxs), 0.5, psound->origin);
			}
			else
			{
		...
		if((psound->entity != pplayer) && !(psound->flags & SND_2D) && !(psound->flags & SND_OCCLUSIONLESS))
		{
			if(psound->entity ||  psound->leafnum == 0)
			{
		...
		if(psound->playing)
		{
			if(ShouldPlay(psound, pparams->vieworg, flframetime))
			{
				...
				if(psound->cache->loopbegin > 0)
				{
					int processed;
					alGetSourcei(psound->playing->sourceindex, AL_BUFFERS_PROCESSED, &processed);
					while(processed--)
					{
						ALuint buffer;
						alSourceUnqueueBuffers(psound->playing->sourceindex, 1, &buffer);
						alSourcei(psound->playing->sourceindex, AL_LOOPING, AL_TRUE);
					}
				}
			}
			else
		...
		else if(ShouldPlay(psound, pparams->vieworg, flframetime))
		{
			...
			//JIW - don't update sound if ent is out of range
			if((psound->entity && (psound->entity->index > 0) && psound->entity != gEngfuncs.GetLocalPlayer())
				&& (psound->entity->curstate.messagenum != imsgnum))
			{
				//HACKHACK: kill buggy looping sounds
				if(psound->flags != SND_AMBIENT && (pcache->loopbegin > 0))
				{
					psound->flags |= SND_KILLME;
				}
				continue;
			}
			//JIW
			...
			alSourceQueueBuffers(pplaying->sourceindex, psound->cache->numbuffers, psound->cache->buffers);
			...
			alSourcei(pplaying->sourceindex, AL_LOOPING, (pcache->loopbegin != 0) ? AL_FALSE : AL_TRUE);
	...
	if(m_pMusicTrack.used)
	{
		if(!m_pMusicTrack.source)
		{
			...
			alSourcei(m_pMusicTrack.source, AL_LOOPING, AL_FALSE);
		...
		else if(state == AL_STOPPED)
		{
			if(!(m_pMusicTrack.flags & MUSIC_LOOP))
			{
				...
				//JIW - advance to new song in playlist
				if(m_flUpdateMusic == -1 && g_iPlayListNumItems > 0)
				{
					g_iplPos++;
					if(g_iplPos > g_iPlayListNumItems)
						g_iplPos = 0;
			...
			else
			{
				...
				// reset
				ov_time_seek(&m_pMusicTrack.stream, 0.0f);
				...
				alSourcei(m_pMusicTrack.source, AL_BYTE_OFFSET, 0);
				alSourcePlay(m_pMusicTrack.source);
			}
		}
	...
	//JIW
	else if(m_flUpdateMusic <= gEngfuncs.GetClientTime() && m_flUpdateMusic != -1)
	{
		char mapname[64];
		const char *levelname = gEngfuncs.pfnGetLevelName();
		FilenameFromPath((char*)levelname, mapname);

		char mapmusic[128];
		sprintf(mapmusic,"music/%s.ogg",mapname);

		if(PlayOgg(mapmusic,MUSIC_LOOP))
		{
			gEngfuncs.Con_Printf("Playing mapmusic: %s\n", mapmusic);
		}
		else if(g_iPlayListNumItems > 0)
		{
			gEngfuncs.Con_Printf("Playing playlist slot %d: %s\n", g_iplPos,g_szPlayList[g_iplPos]);
			PlayOgg(g_szPlayList[g_iplPos],0);
		}
...
====================
*/
int CSoundSystem::Stream( ALuint buffer )
{
...
float CSoundSystem::CalcGain( vec3_t vieworg, s_active_t *sound, float multval )
{
	...
	{
		float flrefdist = sound->radius*0.2;
		float fldistance = min((vieworg-sound->origin).Length(), sound->radius);
		float flgain = (1-SE_ROLLOFF_FACTOR*(fldistance-flrefdist)/(sound->radius-flrefdist));

		if(m_pCvarOcclusion->value >= 1)
		{
			if(!(sound->flags & SND_OCCLUSIONLESS) && !(sound->flags & SND_2D))
			{
				pmtrace_t tr;
				gEngfuncs.pEventAPI->EV_SetTraceHull(2);
				gEngfuncs.pEventAPI->EV_PlayerTrace(vieworg, sound->origin, PM_WORLD_ONLY, -1, &tr);
				...
				//JIW - even quieter if underwater while player is out
				if(gEngfuncs.PM_PointContents(tr.endpos, NULL) == CONTENTS_WATER
					&& gEngfuncs.PM_PointContents(vieworg, NULL) != CONTENTS_WATER)
				{
					flgain = flgain*0.35;
				}
			}
		...
		return sound->volume*flgain*multval*m_pCvarVolume->value;
	}
}
...
====================
*/
int CSoundSystem::PlayOgg( char *sample, int flags )
{
	//JIW - make sure we're not sending a borked sample
	if(strlen( sample ) < 1)
		return 0;	
...
//		return;
	...
	if(psound->sent)
	{
		...
		psound->datapos = psample->length*((float)psound->chunk->start/100.0f);
		psound->volume = ((float)psound->chunk->volume/100.0f)*psound->volume;
		psound->pitch = ((float)psound->chunk->pitch/100.0f)*psound->mainpitch;
	}
	...
	else if(entity && psound->radius)
	{
		...
		{
			vec3_t vmins = entity->origin + entity->model->mins;
			vec3_t vmaxs = entity->origin + entity->model->maxs;
			VectorScale((vmins+vmaxs), 0.5, psound->origin);
		}
		else
		{
			VectorCopy(entity->origin, psound->origin);
		}
	}
...
void CSoundSystem::MsgFunc_EmitSound( const char *pszName, int iSize, void *pbuf )
{
	...
	*/
	...
	if(flags & SND_STOP)//JIW
	{
		StopSound(entindex, channel);
		return;
	}
	...
	float volume = READ_FLOAT();//READ_COORD()/100.0f;
	float atten = READ_FLOAT();//READ_COORD()/100.0f;
	int pitch = READ_BYTE();
...
extern "C" void PM_PlaySound( int idx, float *origin, int channel, const char *sample, float volume, float attenuation, int fFlags, int pitch )
{
	...
	//JIW - play sounds for local players!
	if(idx == gEngfuncs.GetLocalPlayer()->index)
		gSoundSystem.PlaySound( (char *)sample, origin, fFlags, channel, volume, pitch, attenuation, pentity );
}
...
===================================
*/
inline void PlaySound( char *szSound, float vol, int channel, int pitch, int flags ) 
{
	cl_entity_t *pplayer = gEngfuncs.GetLocalPlayer();
	gSoundSystem.PlaySound(szSound, NULL, flags|SND_REVERBLESS, channel, vol, pitch, ATTN_NONE, pplayer, pplayer->index); 
}

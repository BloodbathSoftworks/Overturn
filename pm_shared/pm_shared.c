...
extern void PM_PlaySound( int idx, float *origin, int channel, const char *sample, float volume, float attenuation, int fFlags, int pitch );
...
void PM_PlayGroupSound( const char* szValue, int irand, float fvol )
{
	...
	for (i = 0; szValue[i]; i++)
	{
		if (szValue[i] == '?')
		{
			...
			PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, szBuf, fvol, ATTN_NORM, 0, PITCH_NORM );
			return;
		}
	}
	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, szValue, fvol, ATTN_NORM, 0, PITCH_NORM );
}

void PM_PlayStepSound( int step, float fvol, int type )//JIW - added type (0 - step / 1 - jump / 2- land)
{
	...
	if ( pmove->multiplayer && ( !fladder
		&& (type == 0) //JIW
		&& Length( hvel ) <= 180 ) )//JIW//220
		return;
		...
		&& (step != STEP_LADDER))
	{
		...
		case STEP_VENT:
		case STEP_GRATE:
			PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_metal-jump1.wav", 0.5, ATTN_IDLE, 0, PITCH_NORM );
			break;
		case STEP_DIRT:
			PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_dirt-jump1.wav", 0.5, ATTN_IDLE, 0, PITCH_NORM );
			break;
		default:
			PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_step-jump1.wav", 0.5, ATTN_IDLE, 0, PITCH_NORM );
			break;
		}

		pmove->flTimeStepSound = 800;
	}
	else if (type == 2//land
		&& (step != STEP_SLOSH)
		&& (step != STEP_WADE)
		&& (step != STEP_LADDER))
	{
		...
		case STEP_VENT:
		case STEP_GRATE:
			PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_metal-land1.wav", 0.5, ATTN_IDLE, 0, PITCH_NORM );
			break;
		case STEP_DIRT:
			PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_dirt-land1.wav", 0.5, ATTN_IDLE, 0, PITCH_NORM );
			break;
		default:
			PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_step-land1.wav", 0.5, ATTN_IDLE, 0, PITCH_NORM );
			break;
		}
	...
	case STEP_CONCRETE:
		...
		{
		// right foot
		case 0:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_step1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_step3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_step2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_step4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_METAL:
		switch(irand)
		{
		// right foot
		case 0:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_metal1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_metal3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_metal2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_metal4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_DIRT:
		switch(irand)
		{
		// right foot
		case 0:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_dirt1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_dirt3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_dirt2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_dirt4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_VENT:
		switch(irand)
		{
		// right foot
		case 0:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_duct1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_duct3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_duct2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_duct4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_GRATE:
		switch(irand)
		{
		// right foot
		case 0:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_grate1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_grate3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_grate2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_grate4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_TILE:
		...
		{
		// right foot
		case 0:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_tile1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_tile3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_tile2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_tile4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 4: PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_tile5.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_SLOSH:
		switch(irand)
		{
		// right foot
		case 0:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_slosh1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_slosh3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_slosh2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_slosh4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_WADE:
		if ( iSkipStep == 0 )
		...
		{
		// right foot
		case 0:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_wade1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_wade2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_wade3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_wade4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_LADDER:
		switch(irand)
		{
		// right foot
		case 0:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_ladder1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_ladder3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_ladder2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_ladder4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	}
...
void PM_UnDuck( void )
{
	...
	if ( pmove->onground != -1 && pmove->flags & FL_DUCKING && pmove->bInDuck == false)//unduck fix
	{
		for ( i = 0; i < 3; i++ )
		{
...
void PM_Jump (void)
{
	...
	{	// swimming, not jumping
		...
		if ( pmove->flSwimTime <= 0 )
		{
			...
			{ 
			case 0:
				PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_wade1.wav", 1, ATTN_NORM, 0, PITCH_NORM );
				break;
			case 1:
				PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_wade2.wav", 1, ATTN_NORM, 0, PITCH_NORM );
				break;
			case 2:
				PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_wade3.wav", 1, ATTN_NORM, 0, PITCH_NORM );
				break;
			case 3:
				PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/pl_wade4.wav", 1, ATTN_NORM, 0, PITCH_NORM );
				break;
			}
		}
	...
	if ( tfc )
	{
		PM_PlaySound( pmove->player_index, pmove->origin, CHAN_BODY, "player/plyrjmp8.wav", 0.5, ATTN_NORM, 0, PITCH_NORM );
	}
	else
	{
		PM_PlayStepSound( PM_MapTextureTypeStepType( pmove->chtexturetype ), 1.0, 1 );
	}
	...
	if ( ( pmove->bInDuck ) || ( pmove->flags & FL_DUCKING ) )
	{
			...
			Length( pmove->velocity ) > 50 )
		{
			pmove->punchangle[0] = -5;
				...
				PM_PlaySound( pmove->player_index, pmove->origin, CHAN_STATIC, "items/longjump.wav", 1.0, ATTN_NORM, 0, 98 );
...
void PM_CheckFalling( void )
{
	if ( pmove->onground != -1 &&
		 !pmove->dead &&
		 pmove->flFallVelocity >= 80 )//PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		float fvol = 0.5;
		...
		else if ( pmove->flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED )
		{
			...
			//{
			//case 0:
				//PM_PlaySound( pmove->player_index, pmove->origin, CHAN_VOICE, "player/pl_fallpain2.wav", 1, ATTN_NORM, 0, PITCH_NORM );
				//break;
			//case 1:
				PM_PlaySound( pmove->player_index, pmove->origin, CHAN_VOICE, "player/pl_fallpain3.wav", 1, ATTN_NORM, 0, PITCH_NORM );
			//	break;
			//}
			fvol = 1.0;
		}
		else if ( pmove->flFallVelocity > 180)//PLAYER_MAX_SAFE_FALL_SPEED / 2 )
		{
			qboolean tfc = false;
			tfc = atoi( pmove->PM_Info_ValueForKey( pmove->physinfo, "tfc" ) ) == 1 ? true : false;

			//if ( tfc )
			if ( pmove->flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED / 2 )
			{
				PM_PlaySound( pmove->player_index, pmove->origin, CHAN_VOICE, "player/pl_fallpain2.wav", 1, ATTN_NORM, 0, PITCH_NORM );
				fvol = 0.85;
			}
		...
		if ( fvol > 0.0 )
		{
			...
			PM_PlayStepSound( PM_MapTextureTypeStepType( pmove->chtexturetype ), fvol, 2 );//JIW - added type
